#!/usr/bin/env python3
"""Drive the Hermes TUI under SHUOZI_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.shuozi/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with SHUOZI_TUI_RESUME set so it
bypasses the shuozi_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  SHUOZI_PERF_LOG     (default ~/.shuozi/perf.log)
  SHUOZI_PERF_NODE    (default node from $PATH)
  SHUOZI_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests SHUOZI_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_shuozi_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("SHUOZI_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".hermes"

DEFAULT_TUI_DIR = Path(
    os.environ.get("SHUOZI_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("SHUOZI_PERF_LOG", str(get_shuozi_home() / "perf.log")))
DEFAULT_STATE_DB = get_shuozi_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — SHUOZI_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["SHUOZI_DEV_PERF"] = "1"
    env["SHUOZI_DEV_PERF_MS"] = str(args.threshold_ms)
    env["SHUOZI_DEV_PERF_LOG"] = str(log)
    env["SHUOZI_TUI_RESUME"] = sid
    ...
    node = os.environ.get("SHUOZI_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="SHUOZI_DEV_PERF_MS (0 = capture all)")
