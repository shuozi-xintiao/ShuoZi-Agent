#!/usr/bin/env bash
# Canonical test runner for shuozi-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.shuozi/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.shuozi/shuozi-agent/venv"; do
...
if [ -f "$HOME/.shuozi/pytest_live_guard.py" ]; then
