...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/shuozi-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/shuozi-agent.git"
SHUOZI_HOME="${SHUOZI_HOME:-$HOME/.hermes}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SHUOZI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SHUOZI_INSTALL_DIR"
...
#   code at /usr/local/lib/shuozi-agent, command at /usr/local/bin/hermes,
#   data still at /root/.hermes (SHUOZI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SHUOZI_HOME/.no-bundled-skills so future"
            echo "                   'shuozi update' runs never inject bundled skills either"
            ...
            echo "                   default (non-root):  ~/.shuozi/shuozi-agent"
            echo "                   default (root, Linux): /usr/local/lib/shuozi-agent"
            echo "  --shuozi-home PATH  Data directory (default: ~/.shuozi, or \$SHUOZI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/shuozi-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SHUOZI_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SHUOZI_HOME/shuozi-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `shuozi update`
...
emit_manifest() {
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download ShuoZi Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hermes command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SHUOZI_HOME/shuozi-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SHUOZI_HOME/shuozi-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/shuozi-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SHUOZI_HOME/shuozi-agent — then preserve it)
#
# Always no-op when the user set --dir or $SHUOZI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SHUOZI_HOME.
    if is_termux; then
        INSTALL_DIR="$SHUOZI_HOME/shuozi-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SHUOZI_HOME/shuozi-agent/.git" ]; then
            INSTALL_DIR="$SHUOZI_HOME/shuozi-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/shuozi-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/shuozi-agent"
        ...
        log_info "  Data:    $SHUOZI_HOME (unchanged)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://shuozi-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $SHUOZI_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `shuozi update` stay in sync.
    local _managed_uv="$SHUOZI_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SHUOZI_HOME/bin ..."
    mkdir -p "$SHUOZI_HOME/bin"
    ...
    # directly into $SHUOZI_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SHUOZI_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$SHUOZI_HOME/node/bin/node" ] && node_satisfies_build "$("$SHUOZI_HOME/node/bin/node" --version)"; then
        export PATH="$SHUOZI_HOME/node/bin:$PATH"
        log_success "Node.js $("$SHUOZI_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.shuozi/node/..."
    ...
    # Place into ~/.shuozi/node/ and symlink binaries into the same bin dir
    # the hermes command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SHUOZI_HOME/node"
    mkdir -p "$SHUOZI_HOME"
    mv "$extracted_dir" "$SHUOZI_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SHUOZI_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SHUOZI_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SHUOZI_HOME/node/bin/npx"  "$node_link_dir/npx"

    export PATH="$SHUOZI_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SHUOZI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.shuozi/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "ShuoZi Agent itself does not require or retain root access."
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"shuozi-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        SHUOZI_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        SHUOZI_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$SHUOZI_BIN" ]; then
            log_warn "shuozi not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$SHUOZI_BIN" ]; then
        log_warn "shuozi entry point not found at $SHUOZI_BIN"
    ...
    # Older installs created this path as a symlink to $SHUOZI_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SHUOZI_BIN"` self-recurse. (#21454)
...
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "shuozi command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "shuozi not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# ShuoZi Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# ShuoZi Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# ShuoZi Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # Create ~/.shuozi directory structure (config at top level, code in subdir)
    mkdir -p "$SHUOZI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.shuozi/.env (top level, easy to find)
    if [ ! -f "$SHUOZI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SHUOZI_HOME/.env"
            log_success "Created ~/.shuozi/.env from template"
        else
            touch "$SHUOZI_HOME/.env"
            log_success "Created ~/.shuozi/.env"
        fi
    else
        log_info "~/.shuozi/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SHUOZI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.shuozi/config.yaml (top level, easy to find)
    if [ ! -f "$SHUOZI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SHUOZI_HOME/config.yaml"
            log_success "Created ~/.shuozi/config.yaml from template"
        fi
    else
        log_info "~/.shuozi/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$SHUOZI_HOME/SOUL.md" ]; then
        cat > "$SHUOZI_HOME/SOUL.md" << 'SOUL_EOF'
# ShuoZi Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.shuozi/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.shuozi/"

    # Seed bundled skills into ~/.shuozi/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `shuozi update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'shuozi update'." \
            > "$SHUOZI_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SHUOZI_HOME/.no-bundled-skills"
        log_info "  Future 'shuozi update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.shuozi/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.shuozi/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SHUOZI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SHUOZI_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.shuozi/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$SHUOZI_HOME/.env"
    ...
    {
        echo ""
        echo "# ShuoZi Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `shuozi update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'shuozi setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$SHUOZI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'shuozi whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SHUOZI_CMD="$(get_hermes_command_path)"
                $SHUOZI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'shuozi whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'shuozi gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SHUOZI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SHUOZI_CMD gateway start 2>/dev/null; then
            ...
            nohup $SHUOZI_CMD gateway > "$SHUOZI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.shuozi/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SHUOZI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SHUOZI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SHUOZI_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'shuozi' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'shuozi' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'shuozi' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SHUOZI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SHUOZI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SHUOZI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$SHUOZI_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$SHUOZI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    SHUOZI_CMD="$(command -v hermes 2>/dev/null || echo "")"
    if [ -n "$SHUOZI_CMD" ]; then
        log_info "Running hermes setup..."
        "$SHUOZI_CMD" setup
    else
        log_warn "shuozi command not found on PATH"
...
# _purge_electron_build_cache() used by `shuozi desktop`; install.sh was the only
...
install_desktop() {
    ...
    #         retry (matches install.ps1 / `shuozi desktop`),
    ...
    # checkout stays clean for the next `shuozi update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $SHUOZI_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$SHUOZI_HOME/.install_method"
            ;;
        *)
