...
# Bootstrap Open WebUI against ShuoZi Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.shuozi/.env has API server settings
...
#   SHUOZI_API_PORT=8642
#   SHUOZI_API_HOST=127.0.0.1
#   SHUOZI_API_MODEL_NAME='ShuoZi Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-ShuoZi Agent WebUI}"
...
SHUOZI_ENV_FILE="${SHUOZI_ENV_FILE:-$HOME/.shuozi/.env}"
SHUOZI_API_PORT="${SHUOZI_API_PORT:-8642}"
SHUOZI_API_HOST="${SHUOZI_API_HOST:-127.0.0.1}"
SHUOZI_API_CONNECT_HOST="${SHUOZI_API_CONNECT_HOST:-127.0.0.1}"
SHUOZI_API_MODEL_NAME="${SHUOZI_API_MODEL_NAME:-ShuoZi Agent}"
SHUOZI_API_BASE_URL="http://${SHUOZI_API_CONNECT_HOST}:${SHUOZI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-hermes.sh"
LOG_DIR="$HOME/.shuozi/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$SHUOZI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$SHUOZI_ENV_FILE" 2>/dev/null || true
}
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to ShuoZi Agent
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.shuozi/logs/openwebui.log
StandardError=append:%h/.shuozi/logs/openwebui.error.log
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$SHUOZI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Hermes API server is configured...'
  upsert_env API_SERVER_ENABLED true "$SHUOZI_ENV_FILE"
  upsert_env API_SERVER_HOST "$SHUOZI_API_HOST" "$SHUOZI_ENV_FILE"
  upsert_env API_SERVER_PORT "$SHUOZI_API_PORT" "$SHUOZI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$SHUOZI_API_MODEL_NAME" "$SHUOZI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$SHUOZI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Hermes gateway so API server settings take effect...'
  hermes gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${SHUOZI_API_CONNECT_HOST}:${SHUOZI_API_PORT}/health" >/dev/null; then
  ...
  curl -fsS "http://${SHUOZI_API_CONNECT_HOST}:${SHUOZI_API_PORT}/health" >/dev/null
  ...
  log "Hermes API endpoint: ${SHUOZI_API_BASE_URL}"
