...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with shuozi-agent)
2. User plugins: ``$SHUOZI_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SHUOZI_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SHUOZI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from shuozi_constants import get_shuozi_home

        d = get_shuozi_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SHUOZI_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SHUOZI_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with shuozi-agent.
    ...
    # 2. User plugins — under $SHUOZI_HOME/plugins/model-providers/<name>/.
