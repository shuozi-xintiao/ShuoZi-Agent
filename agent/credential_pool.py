...
from shuozi_cli.config import load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``shuozi model`` / ``shuozi auth`` which
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `shuozi model` / `shuozi auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `shuozi model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``shuozi auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `shuozi auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `shuozi auth remove <provider> <N>` is stable across all source types.
    try:
        from shuozi_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `shuozi setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.shuozi/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `shuozi setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `shuozi auth add nous --label <name>`).  Fall back to the
        ...
        try:
            from shuozi_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.shuozi/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `shuozi auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `shuozi auth remove openai-codex` marks
        ...
        # When the user logs in via ``shuozi model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``shuozi auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from shuozi_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.shuozi/.env over os.environ — the user's config file is the
    ...
    # Honour user suppression — `shuozi auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.shuozi/.env.
    ...
    try:
        from shuozi_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from shuozi_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.shuozi/.env over os.environ
