...
Storage
-------
Bundles live in ``~/.shuozi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SHUOZI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``shuozi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SHUOZI_HOME.

    Honors ``SHUOZI_BUNDLES_DIR`` for tests; falls back to
    ``<SHUOZI_HOME>/skill-bundles``.
    """
    override = os.environ.get("SHUOZI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_shuozi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `shuozi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
