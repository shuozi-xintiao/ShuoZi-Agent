...
Storage (``~/.shuozi/auth/google_oauth.json``, chmod 0o600):
...
from shuozi_constants import get_shuozi_home, secure_parent_dir
...
#
# Resolution order:
#   1. SHUOZI_GEMINI_CLIENT_ID / SHUOZI_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "SHUOZI_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "SHUOZI_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "SHUOZI_HEADLESS")
...
def _credentials_path() -> Path:
    return get_shuozi_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "  2. Set SHUOZI_GEMINI_CLIENT_ID and SHUOZI_GEMINI_CLIENT_SECRET in ~/.shuozi/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `shuozi auth add google-gemini-cli` first.",
