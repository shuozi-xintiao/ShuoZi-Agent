...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.shuozi/kanban.db)
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on SHUOZI_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.shuozi/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on SHUOZI_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "ShuoZi Agent's tool surface, exposed for use inside a Codex "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    os.environ.setdefault("SHUOZI_QUIET", "1")
    os.environ.setdefault("SHUOZI_REDACT_SECRETS", "true")
