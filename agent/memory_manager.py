...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SHUOZI_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``shuozi_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_shuozi_home()`` themselves.
        """
        if "shuozi_home" not in kwargs:
            from shuozi_constants import get_shuozi_home
            kwargs["shuozi_home"] = str(get_shuozi_home())
        for provider in self._providers:
            try:
