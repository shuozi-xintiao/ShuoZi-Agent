...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - shuozi_home (str): The active SHUOZI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.shuozi``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "shuozi").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'shuozi memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], shuozi_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'shuozi memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``shuozi_home`` is the active SHUOZI_HOME directory path.
