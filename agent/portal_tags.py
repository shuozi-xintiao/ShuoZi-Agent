...
Nous can attribute usage to ShuoZi Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=shuozi-agent",
        "client=hermes-client-v<__version__>",
    ]

The version is sourced live from ``shuozi_cli.__version__`` so it auto-aligns
...
``shuozi_cli.__version__`` is the canonical source of truth.
"""
...
def _shuozi_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``shuozi_cli`` cannot be imported (should
...
def shuozi_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=hermes-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=hermes-client-v{_shuozi_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=shuozi-agent", shuozi_client_tag()]
