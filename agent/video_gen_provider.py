...
as ``kind: backend``) or ``~/.shuozi/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``shuozi tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SHUOZI_HOME/cache/videos/``, creating parents as needed."""
    from shuozi_constants import get_shuozi_home

    path = get_shuozi_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SHUOZI_HOME/cache/videos/``.
