...
upstream prefix cache warm.  See ``shuozi-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # Some execution modes (cron) still want SHUOZI_HOME persona while keeping
    ...
    # Pointer to the shuozi-agent skill + docs for user questions about Hermes itself.
    stable_parts.append(SHUOZI_AGENT_HELP_GUIDANCE)
    ...
    # SHUOZI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # under so it doesn't conflate ~/.shuozi/skills/ (default profile) with
    # ~/.shuozi/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            ...
            "under ~/.shuozi/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            ...
            f"and writes ~/.shuozi/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.shuozi/skills/, ~/.shuozi/plugins/, "
            f"~/.shuozi/cron/, ~/.shuozi/memories/ — those belong to a "
