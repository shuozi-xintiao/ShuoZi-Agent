...
# heuristics. See https://github.com/NousResearch/shuozi-agent/issues/38389
#
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the shuozi compressor emits).
    """
    if not messages:
        return messages
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # user's configured timezone via shuozi_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _shuozi_now().strftime("%Y-%m-%d")
