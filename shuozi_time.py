...
Resolution order:
  1. ``SHUOZI_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.shuozi/config.yaml``
...
from shuozi_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("SHUOZI_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``SHUOZI_TIMEZONE`` update) to force ``get_timezone()`` /
