"""CLI handlers for the ``shuozi proxy`` subcommand."""
...
from shuozi_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "shuozi proxy requires aiohttp. Install one of:\n"
        "  pip install 'shuozi-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"shuozi auth add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: shuozi proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``shuozi proxy <subcommand>``."""
    ...
    print(
        "shuozi proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  shuozi proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  shuozi proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  shuozi proxy providers\n"
