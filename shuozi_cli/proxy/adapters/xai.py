...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "shuozi auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`shuozi auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`shuozi auth reset xai-oauth` or re-authenticate with "
                    "`shuozi auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `shuozi auth add xai-oauth --type oauth`."
            )
