"""
Configuration management for ShuoZi Agent.

Config files are stored in ~/.shuozi/ for easy access:
- ~/.shuozi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.shuozi/.env         - API keys and secrets

This module provides:
- shuozi config          - Show current configuration
- shuozi config edit     - Open config in editor
- shuozi config set      - Set a specific value
- shuozi config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``shuozi config set`` (which rewrites
    ...
    ``config.yaml`` in place: shuozi never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.shuozi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``shuozi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  shuozi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``shuozi update``.
...
# * ``SHUOZI_HOME`` / ``SHUOZI_PROFILE`` / ``SHUOZI_CONFIG`` /
#   ``SHUOZI_ENV`` — ShuoZi runtime location flags. Writing these into
...
#
# IMPORTANT: ``SHUOZI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SHUOZI_GEMINI_CLIENT_ID,
# SHUOZI_LANGFUSE_PUBLIC_KEY, SHUOZI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # ShuoZi runtime location — never via dashboard env writer.
    # NOT a SHUOZI_* blanket: integration credentials (SHUOZI_GEMINI_*,
    # SHUOZI_LANGFUSE_*, SHUOZI_SPOTIFY_*, ...) ARE allowed.
    "SHUOZI_HOME", "SHUOZI_PROFILE", "SHUOZI_CONFIG", "SHUOZI_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or ShuoZi runtime location "
            "(SHUOZI_HOME, SHUOZI_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SHUOZI_TOOL_PROGRESS", "SHUOZI_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `shuozi plugins enable
    # observability/langfuse` or `shuozi tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SHUOZI_LANGFUSE_ENV",
    "SHUOZI_LANGFUSE_RELEASE",
    "SHUOZI_LANGFUSE_SAMPLE_RATE",
    "SHUOZI_LANGFUSE_MAX_CHARS",
    "SHUOZI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from shuozi_cli.colors import Colors, color
from shuozi_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SHUOZI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_shuozi_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the SHUOZI_MANAGED env var (set by the systemd service),
    or a .managed marker file in SHUOZI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade shuozi-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.shuozi/.install_method`` file (written by installers)
    2. SHUOZI_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/shuozi-agent`` image stamps ``docker``
    ...
    so ``shuozi update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_shuozi_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.shuozi/.install_method."""
    stamp = get_shuozi_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    ...
    ``uv tool install shuozi-agent`` places the install at
    ``.../uv/tools/shuozi-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``shuozi-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hermes is a regular pip/venv install, causing
    ``shuozi update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/shuozi-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade shuozi-agent"
    if method == "docker":
        return "docker pull nousresearch/shuozi-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade shuozi-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade shuozi-agent"
        return "pip install --upgrade shuozi-agent"
    return "shuozi update"
...
# Long-form text for ``shuozi update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# shuozi_cli/main.py; lives here so the wording stays consistent and we
...
✗ ``shuozi update`` doesn't apply inside the Docker container.

ShuoZi Agent runs as a published image (nousresearch/shuozi-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/shuozi-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate shuozi-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/shuozi-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/shuozi-agent/tags
  • Your config and session history live under ``$SHUOZI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``shuozi update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("SHUOZI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(SHUOZI_MANAGED={env_hint}).\n"
            "Edit services.shuozi-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SHUOZI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or SHUOZI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SHUOZI_DEV") == "1":
        return None

    from shuozi_constants import is_container
    if is_container():
        return None

    container_mode_file = get_shuozi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "shuozi-agent")
    exec_user = info.get("exec_user", "shuozi")
...
# Re-export from shuozi_constants — canonical definition lives there.
from shuozi_constants import get_shuozi_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_shuozi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_shuozi_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SHUOZI_UID / SHUOZI_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level SHUOZI_HOME once, but
    subdirectories created at runtime by ``ensure_shuozi_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SHUOZI_UID", "").strip()
    gid_str = os.environ.get("SHUOZI_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``SHUOZI_UID:SHUOZI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_shuozi_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the shuozi group can
    share state with the gateway service.

    The mode can be overridden via the SHUOZI_HOME_MODE environment variable
    (e.g. SHUOZI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SHUOZI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SHUOZI_UID``/``SHUOZI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SHUOZI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("SHUOZI_CONTAINER") or os.environ.get("SHUOZI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SHUOZI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SHUOZI_HOME if the user doesn't have one yet."""
...
def ensure_shuozi_home():
    """Ensure ~/.shuozi directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_shuozi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_shuozi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SHUOZI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The SHUOZI_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        #  "/home/user/.shuozi/cache/documents:/output"]
    ...
    # Opt in via ``shuozi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.shuozi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: shuozi sweeps the checkpoint base at startup
    ...
    "auxiliary": {
        ...
        # Invoked by ``shuozi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``shuozi kanban decompose`` and the kanban
        ...
        # ``shuozi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `shuozi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `shuozi chat`) launches by default:
        ...
        # REPL and `--tui` (or SHUOZI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `shuozi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `shuozi -c` muscle memory.  Default off so existing
        # users aren't surprised.  SHUOZI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `shuozi --tui` drops a one-time hint
    ...
    "dashboard": {
        ...
        # ``SHUOZI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SHUOZI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SHUOZI_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``SHUOZI_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.shuozi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.shuozi/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.shuozi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SHUOZI_SKILL_DIR} and ${SHUOZI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `shuozi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `shuozi update`, so pruning them only
        ...
        # skipped), snapshot ~/.shuozi/skills/ into
        # ~/.shuozi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `shuozi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *ShuoZi Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SHUOZI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.shuozi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SHUOZI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `shuozi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``shuozi_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # Also overridable via SHUOZI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `shuozi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # decomposition is manual via `shuozi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     shuozi-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.shuozi/logs/.
    ...
    # update model picker lists without shipping a shuozi-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://shuozi-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `shuozi model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.shuozi/.env,
        ...
        # user. Bridged to SHUOZI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # cache (~/.shuozi/cache/{documents,images,audio,video,screenshots})
        ...
        # to SHUOZI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SHUOZI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SHUOZI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.shuozi/state.db.
    ...
    "sessions": {
        ...
        # default of ``shuozi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.shuozi/sessions/session_{sid}.json`` on every turn
    ...
    # ``shuozi update`` behaviour.
    "updates": {
        # Run a full ``shuozi backup``-style zip of SHUOZI_HOME before every
        # ``shuozi update``.  Backups land in ``<SHUOZI_HOME>/backups/`` and
        # can be restored with ``shuozi import <path>``.  Off by default —
        # on large SHUOZI_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `shuozi update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<SHUOZI_HOME>/lsp/bin/`` on first use.
    ...
    # `shuozi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.shuozi/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.shuozi/.env (or your shell) and never in config.yaml.
            ...
            # ~/.shuozi/bin/ on first use.  When False you must install
            ...
            # `shuozi secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'shuozi model' for endpoint-specific config)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SHUOZI_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'shuozi-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # SHUOZI_TOOL_PROGRESS and SHUOZI_TOOL_PROGRESS_MODE are deprecated —
    ...
    "SHUOZI_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `shuozi update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``shuozi_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'shuozi setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'shuozi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SHUOZI_HOME", "~/.shuozi")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SHUOZI_TOOL_PROGRESS")
            old_mode = get_env_value("SHUOZI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SHUOZI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SHUOZI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SHUOZI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SHUOZI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SHUOZI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_shuozi_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `shuozi plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `shuozi curator status` has no
    ...
    #   3. Creates `~/.shuozi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_shuozi_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_shuozi_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: shuozi config set <key> <value>")
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _shuozi_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.shuozi/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.shuozi/config.yaml.
    ...
    (which change ``SHUOZI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — shuozi auth) — OpenAI Codex
#   nous         (OAuth — shuozi auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.shuozi/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.shuozi/.env.
    ...
    interactive menu render (`shuozi tools`, `shuozi setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `shuozi tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.shuozi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.shuozi/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.shuozi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.shuozi/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.shuozi/.env or environment."""
...
def show_config():
    ...
    from shuozi_cli.auth import get_anthropic_key
    ...
    # Warn on stale SHUOZI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SHUOZI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SHUOZI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'shuozi doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  shuozi config edit     # Edit config file", Colors.DIM))
    print(color("  shuozi config set <key> <value>", Colors.DIM))
    print(color("  shuozi setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: shuozi config set <key> <value>")
            print()
            print("Examples:")
            print("  shuozi config set model anthropic/claude-sonnet-4")
            print("  shuozi config set terminal.backend docker")
            print("  shuozi config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'shuozi config migrate' to add them")
        ...
        print("  shuozi config           Show current configuration")
        print("  shuozi config edit      Open config in editor")
        print("  shuozi config set <key> <value>   Set a config value")
        print("  shuozi config check     Check for missing/outdated config")
        print("  shuozi config migrate   Update config with new options")
        print("  shuozi config path      Show config file path")
        print("  shuozi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``shuozi config`` UI so users
