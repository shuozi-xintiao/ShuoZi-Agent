"""
ShuoZi Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.shuozi/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing shuozi-agent or shuozi PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# ShuoZi Agent" comment and following line
                if '# ShuoZi Agent' in line or '# shuozi-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('shuozi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'shuozi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the shuozi wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "shuozi",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains shuozi_cli reference)
                content = wrapper.read_text()
                if 'shuozi_cli' in content or 'shuozi-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(shuozi_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``shuozi`` command:
    ...
    """
    node_dir = (shuozi_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``shuozi gateway run`` processes
    ...
    try:
        from shuozi_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from shuozi_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SHUOZI_HOME`` and ``SHUOZI_GIT_BASH_PATH``
...
#   4. On the ``shuozi dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(shuozi_home: Path) -> list[str]:
    """Path-entry substrings that identify Hermes-owned User-PATH entries."""
    root = str(shuozi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare shuozi-agent install dir.
    markers = [root + "\\shuozi-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SHUOZI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "shuozi".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-hermes as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(shuozi_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(shuozi_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete SHUOZI_HOME and SHUOZI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SHUOZI_HOME", "SHUOZI_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(shuozi_home: Path) -> list[Path]:
...
def _is_default_shuozi_home(shuozi_home: Path) -> bool:
    """Return True when ``shuozi_home`` points at the default (non-profile) root."""
    try:
        from shuozi_constants import get_default_hermes_root
        return shuozi_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SHUOZI_HOME directory.

    We shell out to ``shuozi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SHUOZI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m shuozi_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "shuozi_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's SHUOZI_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``shuozi uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SHUOZI_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from shuozi_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(shuozi_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {shuozi_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The ShuoZi Agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(shuozi_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The ShuoZi Agent at {shuozi_home / 'shuozi-agent'}")
        print(f"  • Your config, sessions, and secrets under {shuozi_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The ShuoZi Agent is still installed. Run 'shuozi' to use the CLI,")
    print("or 'shuozi uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.shuozi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.shuozi/ for future reinstall
    """
    project_root = get_project_root()
    shuozi_home = get_shuozi_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SHUOZI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_shuozi_home(shuozi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.shuozi data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ ShuoZi Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {shuozi_home / 'config.yaml'}")
    print(f"  Secrets: {shuozi_home / '.env'}")
    print(f"  Data:    {shuozi_home / 'cron/'}, {shuozi_home / 'sessions/'}, {shuozi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SHUOZI_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    ``shuozi`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SHUOZI_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in shuozi_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(shuozi_home))))
        ...
        log_info("Removing SHUOZI_HOME / SHUOZI_GIT_BASH_PATH User env vars...")
    ...
    log_info("Removing shuozi command...")
    ...
    removed_node_links = remove_node_symlinks(shuozi_home)
    ...
    #     step-5 rmtree(shuozi_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SHUOZI_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from shuozi_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(shuozi_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.shuozi/, just remove the shuozi-agent subdir
            if shuozi_home in project_root.parents or project_root.parent == shuozi_home:
    ...
    #     under SHUOZI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(shuozi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(shuozi_home)
    ...
    # 5. Optionally remove ~/.shuozi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SHUOZI_HOME dirs live under
        ...
        try:
            if shuozi_home.exists():
                shutil.rmtree(shuozi_home)
                log_success(f"Removed {shuozi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {shuozi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {shuozi_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://shuozi-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://shuozi-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using ShuoZi Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m shuozi_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import shuozi_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``hermes_constants`` + ``shuozi_cli.colors``
    (and lazily ``shuozi_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m shuozi_cli.uninstall")
    parser.add_argument(
        "--mode",
