"""`shuozi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.shuozi/checkpoints/``.  Actions:

    shuozi checkpoints               # same as `status`
    shuozi checkpoints status        # total size, project count, breakdown
    shuozi checkpoints list          # per-project checkpoint counts + workdir
    shuozi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    shuozi checkpoints clear [-f]    # nuke the entire base (asks first)
    shuozi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    shuozi checkpoints
    shuozi checkpoints prune --retention-days 3 --max-size-mb 200
    shuozi checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: shuozi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``shuozi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `shuozi checkpoints` → status
