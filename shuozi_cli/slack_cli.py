"""``shuozi slack ...`` CLI subcommands.

Today only ``shuozi slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ shuozi slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    pasting.
    """
    from shuozi_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your ShuoZi Agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``shuozi_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SHUOZI_HOME/slack-manifest.json``)
    ...
    description = getattr(args, "description", None) or "Your ShuoZi Agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SHUOZI_HOME") or str(Path.home() / ".hermes")) / "slack-manifest.json"
        else:
