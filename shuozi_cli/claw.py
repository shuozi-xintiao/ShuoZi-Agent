"""shuozi claw — OpenClaw migration commands.

Usage:
    shuozi claw migrate              # Preview then migrate (always shows preview first)
    shuozi claw migrate --dry-run    # Preview only, no changes
    shuozi claw migrate --yes        # Skip confirmation prompt
    shuozi claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    shuozi claw migrate --no-backup  # Skip pre-migration snapshot
    shuozi claw cleanup              # Archive leftover OpenClaw directories
    shuozi claw cleanup --dry-run    # Preview what would be archived
"""
...
from shuozi_cli.config import get_shuozi_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'shuozi gateway stop'.")
    print()
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: shuozi claw <command> [options]")
        print()
        print("Commands:")
        ...
        print()
        print("Run 'shuozi claw <command> --help' for options.")
...
def _cmd_migrate(args):
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: shuozi claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {shuozi_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.shuozi/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: shuozi claw migrate --yes")
            return
    ...
    if not no_backup:
        try:
            from shuozi_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(shuozi_home=shuozi_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: shuozi import {backup_archive.name}")
        except Exception as e:
            print()
    ...
    # 'shuozi claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: shuozi claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: shuozi claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  shuozi claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  shuozi claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  shuozi config set OPENROUTER_API_KEY sk-or-v1-...")
