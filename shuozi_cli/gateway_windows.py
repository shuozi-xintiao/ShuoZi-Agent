...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m shuozi_cli.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "ShuoZi Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    ...
    from shuozi_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "shuozi_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("SHUOZI_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("SHUOZI_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("SHUOZI_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["SHUOZI_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["SHUOZI_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["SHUOZI_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("SHUOZI_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("SHUOZI_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("SHUOZI_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during shuozi_cli boot.
    from shuozi_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<SHUOZI_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hermes installs stay self-contained).
    """
    _assert_windows()
    from shuozi_cli.config import get_shuozi_home

    script_dir = Path(get_shuozi_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``SHUOZI_HOME`` whenever it
    ...
    source checkout only if ``SHUOZI_HOME`` cannot be resolved yet.
    """
    from shuozi_cli.config import get_shuozi_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SHUOZI_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m shuozi_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "SHUOZI_HOME={shuozi_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "SHUOZI_GATEWAY_DETACHED=1"')
    ...
    prog_args = [pythonw_path, "-m", "shuozi_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    shuozi_home = str(Path(get_shuozi_home()).resolve())
    profile_arg = _profile_arg(shuozi_home)

    content = _build_gateway_cmd_script(python_path, working_dir, shuozi_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "shuozi_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m shuozi_cli.main gateway run``
    ...
    log_dir = Path(get_shuozi_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("SHUOZI_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("SHUOZI_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from shuozi_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `shuozi -p <profile> gateway install` must be safe.
    ...
    from shuozi_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"shuozi {profile_arg} gateway start" if profile_arg else "shuozi gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: shuozi gateway start")
        return
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                ...
                if start_now:
                    ...
                    print("  Approve the Windows UAC prompt, then run: shuozi gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: shuozi gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from shuozi_cli.config import get_shuozi_home
        print(f"    type {Path(get_shuozi_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_shuozi_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from shuozi_cli.config import get_shuozi_home

    shuozi_home = Path(get_shuozi_home()).resolve()
    print()
    print("Next steps:")
    print("  shuozi gateway status                      # Check status")
    print(f"  type {shuozi_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_shuozi_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: shuozi gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: shuozi gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from shuozi_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
