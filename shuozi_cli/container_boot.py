...
``$SHUOZI_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $SHUOZI_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$SHUOZI_HOME``,
    not under ``profiles/``). The dispatcher in ``shuozi_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``shuozi gateway start`` (no ``-p``) targets. Without it,
    bare ``shuozi gateway start`` inside the container would land on
    ...
    ``$SHUOZI_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        shuozi_home: The container's SHUOZI_HOME (typically /opt/data).
            Profiles live under ``<shuozi_home>/profiles/<name>/``;
            the default profile lives at ``<shuozi_home>`` itself.
    ...
    # ``shuozi gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(shuozi_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(shuozi_home)
    ...
    profiles_root = shuozi_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `shuozi profile create` (config.yaml
            # is not — that comes later via `shuozi setup`). Use it as the
            ...
            # via ``shuozi -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(shuozi_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = shuozi_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("SHUOZI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "shuozi":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `shuozi -p <profile> gateway start`
        ...
        # (all dispatched as the shuozi user) won't hit EACCES. See
...
def _write_reconcile_log(
    shuozi_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $SHUOZI_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    shuozi_home = Path(os.environ.get("SHUOZI_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        shuozi_home=shuozi_home, scandir=scandir,
    )
    for a in actions:
        print(
