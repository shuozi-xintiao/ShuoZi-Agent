"""
Baked-in build metadata for ShuoZi Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``shuozi_cli/dump.py`` and ``shuozi_cli/banner.py``).  That doesn't work inside
...
To make ``shuozi dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$SHUOZI_GIT_SHA``
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``SHUOZI_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    written by the Dockerfile's ``SHUOZI_GIT_SHA`` build-arg and contains
