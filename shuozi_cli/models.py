...
Add, remove, or reorder entries here — both `shuozi setup` and
...
_SHUOZI_USER_AGENT = f"hermes-cli/{_SHUOZI_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `shuozi model`
    flow without maintaining a separate static list.
    """
    from shuozi_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SHUOZI_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SHUOZI_HOME/models_dev_cache.json directly (no network) so this
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_shuozi_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SHUOZI_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  shuozi model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `shuozi model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `shuozi model` TUI
...
#
# Some vendors expose several ShuoZi provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `shuozi model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``shuozi model``, the
...
# path. The *interactive* default (GUI onboarding / ``shuozi model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in shuozi_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``shuozi auth add openai-codex`` without
    ``shuozi model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``shuozi model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    shuozi-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from shuozi_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — shuozi-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from shuozi_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``shuozi model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from shuozi_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from shuozi_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``shuozi_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``shuozi auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.shuozi/.env``.
    ...
    """
    try:
        from shuozi_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from shuozi_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from shuozi_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``shuozi model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $SHUOZI_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from shuozi_constants import get_shuozi_home
    return get_shuozi_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SHUOZI_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SHUOZI_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SHUOZI_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from shuozi_constants import get_shuozi_home
    return get_shuozi_home() / "ollama_cloud_models_cache.json"
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so ShuoZi cannot verify the model name."
