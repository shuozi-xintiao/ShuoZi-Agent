...
* ``shuozi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``shuozi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``shuozi profile``, not a parallel tree):

    shuozi profile install <source> [--name N] [--alias] [--force] [--yes]
    shuozi profile update  <name>  [--force-config] [--yes]
    shuozi profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "shuozi_state.db", "response_store.db",
    ...
    "backups", "cache",
    # Infrastructure
    "shuozi-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``shuozi profile update`` can
...
def plan_install(
    ...
    check_hermes_requires(manifest.hermes_requires, shuozi_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.shuozi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `shuozi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`shuozi profile install <source> --name {canon} --force`."
        )
