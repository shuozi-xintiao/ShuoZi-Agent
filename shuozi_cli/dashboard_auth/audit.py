...
Profile-aware location: ``$SHUOZI_HOME/logs/dashboard-auth.log``.
...
from ``hermes_constants`` or other shuozi_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SHUOZI_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``hermes_constants.get_shuozi_home`` semantics: env var wins,
    else ``~/.shuozi``. A local copy avoids an import cycle with the
    middleware which lives below ``shuozi_cli``.
    """
    home = os.environ.get("SHUOZI_HOME") or str(Path.home() / ".hermes")
