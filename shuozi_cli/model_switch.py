...
- ``shuozi_cli.providers``        -- canonical provider identity + overlays
- ``shuozi_cli.model_normalize``  -- per-provider name formatting
...
_SHUOZI_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with ShuoZi Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"shuozi" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "shuozi" in their tag but are fully tool-capable.
#
...
_NOUS_SHUOZI_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``shuozi config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from shuozi_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'shuozi model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'shuozi doctor' — config issues detected:"
        ...
        from shuozi_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from shuozi_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from shuozi_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in shuozi_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from shuozi_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from shuozi_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from shuozi_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by shuozi provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://shuozi-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (SHUOZI_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from shuozi_cli.auth import _load_auth_store
        ...
        # /model picker sees the SAME list `shuozi model` would build, with
    ...
    from shuozi_cli.providers import SHUOZI_OVERLAYS
    from shuozi_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → ShuoZi provider ID.
    # SHUOZI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hermes IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SHUOZI_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `shuozi model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from shuozi_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or SHUOZI_OVERLAYS (keeps /model in sync
    # with `shuozi model`).
    try:
        from shuozi_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see shuozi_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, shuozi_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`shuozi_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from shuozi_cli.models import fetch_openrouter_models
