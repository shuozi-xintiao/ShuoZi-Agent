...
Usage:
    shuozi                     # Interactive chat (default)
    shuozi chat                # Interactive chat
    shuozi gateway             # Run gateway in foreground
    shuozi gateway start       # Start gateway as service
    shuozi gateway stop        # Stop gateway service
    shuozi gateway status      # Show gateway status
    shuozi gateway install     # Install gateway service
    shuozi gateway uninstall   # Uninstall gateway service
    shuozi setup               # Interactive setup wizard
    shuozi logout              # Clear stored authentication
    shuozi status              # Show status of all components
    shuozi cron                # Manage cron jobs
    shuozi cron list           # List cron jobs
    shuozi cron status         # Check if cron scheduler is running
    shuozi doctor              # Check configuration and dependencies
    shuozi honcho setup                    # Configure Honcho AI memory integration
    shuozi honcho status                   # Show Honcho config and connection status
    shuozi honcho sessions                 # List directory → session name mappings
    shuozi honcho map <name>               # Map current directory to a session name
    shuozi honcho peer                     # Show peer names and dialectic settings
    shuozi honcho peer --user NAME         # Set user peer name
    shuozi honcho peer --ai NAME           # Set AI peer name
    shuozi honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    shuozi honcho mode                     # Show current memory mode
    shuozi honcho mode [hybrid|honcho|local]  # Set memory mode
    shuozi honcho tokens                   # Show token budget settings
    shuozi honcho tokens --context N       # Set session.context() token cap
    shuozi honcho tokens --dialectic N     # Set dialectic result char cap
    shuozi honcho identity                 # Show AI peer identity representation
    shuozi honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    shuozi honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    shuozi version             Show version
    shuozi update              Update to latest version
    shuozi uninstall           Uninstall ShuoZi Agent
    shuozi acp                 Run as an ACP server for editor integration
    shuozi sessions browse     Interactive session picker with search

    shuozi claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: shuozi_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``shuozi_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``shuozi update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``shuozi_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, shuozi crashes on import and the user can't run
# ``shuozi update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import shuozi_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'shuozi' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``shuozi tools`` or
    ...
    try:
        ...
        setproctitle.setproctitle("shuozi")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"shuozi", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"shuozi")
...
# that run *before* shuozi_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SHUOZI_HOME")
        if home:
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``SHUOZI_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("SHUOZI_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``SHUOZI_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SHUOZI_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`shuozi --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from shuozi_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"ShuoZi Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``shuozi --version`` before config/logging imports on Termux."""
    if os.environ.get("SHUOZI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from shuozi_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from shuozi_cli.subcommands.cron import build_cron_parser
from shuozi_cli.subcommands.gateway import build_gateway_parser
from shuozi_cli.subcommands.profile import build_profile_parser
from shuozi_cli.subcommands.model import build_model_parser
from shuozi_cli.subcommands.setup import build_setup_parser
from shuozi_cli.subcommands.postinstall import build_postinstall_parser
from shuozi_cli.subcommands.whatsapp import build_whatsapp_parser
from shuozi_cli.subcommands.slack import build_slack_parser
from shuozi_cli.subcommands.login import build_login_parser
from shuozi_cli.subcommands.logout import build_logout_parser
from shuozi_cli.subcommands.auth import build_auth_parser
from shuozi_cli.subcommands.status import build_status_parser
from shuozi_cli.subcommands.webhook import build_webhook_parser
from shuozi_cli.subcommands.hooks import build_hooks_parser
from shuozi_cli.subcommands.doctor import build_doctor_parser
from shuozi_cli.subcommands.security import build_security_parser
from shuozi_cli.subcommands.dump import build_dump_parser
from shuozi_cli.subcommands.debug import build_debug_parser
from shuozi_cli.subcommands.backup import build_backup_parser
from shuozi_cli.subcommands.import_cmd import build_import_cmd_parser
from shuozi_cli.subcommands.config import build_config_parser
from shuozi_cli.subcommands.version import build_version_parser
from shuozi_cli.subcommands.update import build_update_parser
from shuozi_cli.subcommands.uninstall import build_uninstall_parser
from shuozi_cli.subcommands.dashboard import build_dashboard_parser
from shuozi_cli.subcommands.gui import build_gui_parser
from shuozi_cli.subcommands.logs import build_logs_parser
from shuozi_cli.subcommands.prompt_size import build_prompt_size_parser
from shuozi_cli.subcommands.memory import build_memory_parser
from shuozi_cli.subcommands.acp import build_acp_parser
from shuozi_cli.subcommands.tools import build_tools_parser
from shuozi_cli.subcommands.insights import build_insights_parser
from shuozi_cli.subcommands.skills import build_skills_parser
from shuozi_cli.subcommands.pairing import build_pairing_parser
from shuozi_cli.subcommands.plugins import build_plugins_parser
from shuozi_cli.subcommands.mcp import build_mcp_parser
from shuozi_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (hermes tools, shuozi setup, shuozi model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'shuozi {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any shuozi module import.
#
# Many modules cache SHUOZI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SHUOZI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.shuozi/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SHUOZI_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `shuozi mcp add ... --args <command argv>`.
    ...
    # after the subcommand (`shuozi chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors shuozi_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SHUOZI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.shuozi/profiles/coder or
    # /opt/data/profiles/coder).  If SHUOZI_HOME points to the shuozi root
    # instead (e.g. systemd hardcodes SHUOZI_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `shuozi profile use` and the gateway should honour that choice.
    # See issue #22502.
    shuozi_home_env = os.environ.get("SHUOZI_HOME", "")
    if profile_name is None and shuozi_home_env:
        if Path(shuozi_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the shuozi root
    if profile_name is None:
        try:
            from shuozi_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set SHUOZI_HOME
    if profile_name is not None:
        try:
            from shuozi_cli.profiles import resolve_profile_env

            shuozi_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent shuozi from starting
            print(
...
# Load .env from ~/.shuozi/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from shuozi_cli.config import get_shuozi_home
from shuozi_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SHUOZI_REDACT_SECRETS env
# var BEFORE shuozi_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_shuozi_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "SHUOZI_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SHUOZI_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from shuozi_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from shuozi_constants import apply_ipv4_preference as _apply_ipv4
...
from shuozi_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to shuozi_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (shuozi_cli.main._model_flow_*) keep resolving unchanged.
from shuozi_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_shuozi_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SHUOZI_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SHUOZI_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from shuozi_cli.config import get_env_path, get_shuozi_home, load_config
    from shuozi_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from shuozi_cli.config import DEFAULT_CONFIG
    ...
    from shuozi_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_shuozi_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'shuozi')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo shuozi {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo shuozi {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  shuozi --tui --resume {target}")
    if title:
        print(f'  shuozi --tui -c "{title}"')
    print()
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SHUOZI_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SHUOZI_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SHUOZI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SHUOZI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    shuozi_home = os.environ.get("SHUOZI_HOME") or str(Path.home() / ".hermes")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SHUOZI_HOME": shuozi_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(shuozi_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(shuozi_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if shuozi_cli_dir is None:
        shuozi_cli_dir = Path(__file__).parent
    bundled = shuozi_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SHUOZI_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SHUOZI_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from shuozi_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SHUOZI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SHUOZI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SHUOZI_TUI_DIR (e.g. `unset SHUOZI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("SHUOZI_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from shuozi_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from shuozi_cli.config import apply_terminal_config_to_env
    ...
    env["SHUOZI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SHUOZI_PYTHON_SRC_ROOT"] = os.environ.get(
        "SHUOZI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SHUOZI_PYTHON", sys.executable)
    env.setdefault("SHUOZI_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SHUOZI_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SHUOZI_MODEL"] = model
        env["SHUOZI_INFERENCE_MODEL"] = model
    if provider:
        env["SHUOZI_TUI_PROVIDER"] = provider
        env["SHUOZI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SHUOZI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SHUOZI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SHUOZI_TUI_SKILLS"] = value
    if query:
        env["SHUOZI_TUI_QUERY"] = query
    if image:
        env["SHUOZI_TUI_IMAGE"] = image
    if checkpoints:
        env["SHUOZI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SHUOZI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SHUOZI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SHUOZI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SHUOZI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SHUOZI_ACCEPT_HOOKS"] = "1"
    ...
    # SHUOZI_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `shuozi --tui` try to
    ...
    # still set SHUOZI_TUI_RESUME themselves.
    env.pop("SHUOZI_TUI_RESUME", None)
    if resume_session_id:
        env["SHUOZI_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `shuozi update` so
    ...
    if code == 42:
        from shuozi_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SHUOZI_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``shuozi kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``shuozi kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SHUOZI_KANBAN_BOARD"):
        return
    try:
        from shuozi_cli.kanban_db import get_current_board

        os.environ["SHUOZI_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.shuozi/skills/`` with the bundled skill library on first launch.
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``SHUOZI_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("SHUOZI_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'shuozi sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from shuozi_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from shuozi_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'shuozi doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'shuozi setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from shuozi_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["SHUOZI_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.shuozi/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SHUOZI_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SHUOZI_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SHUOZI_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from shuozi_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from shuozi_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `shuozi gateway` then paid a 30s
    ...
    session_dir = get_shuozi_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: shuozi gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `shuozi gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  shuozi gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ ShuoZi Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  shuozi gateway")
        ...
        print()
        print("  Or install as a service: shuozi gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'shuozi whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``shuozi whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``shuozi_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from shuozi_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from shuozi_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from shuozi_cli.config import stamp_install_method
    from shuozi_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from shuozi_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``shuozi model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from shuozi_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from shuozi_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SHUOZI_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from shuozi_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'shuozi model' for "
                "available providers, or run 'shuozi doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in shuozi_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.shuozi/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.shuozi/.env if the active provider is not 'custom'.
    ...
    """
    from shuozi_cli.config import get_env_value, save_env_value, load_config
...
# `shuozi model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `shuozi model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`shuozi_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from shuozi_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from shuozi_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``shuozi model`` flow, then route aux tasks to them here.
    """
    from shuozi_cli.config import load_config
    from shuozi_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from shuozi_cli.auth import _prompt_model_selection
    from shuozi_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from shuozi_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from shuozi_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from shuozi_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from shuozi_cli.curses_ui import curses_radiolist
...
# downstream call sites read `shuozi_cli.main._PROVIDER_MODELS` directly,
...
# fast paths like `shuozi --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from shuozi_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``shuozi setup`` / ``shuozi model``.
    ...
    recover from a malformed paste without editing ``~/.shuozi/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from shuozi_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from shuozi_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `shuozi setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from shuozi_constants import display_shuozi_home as _dhh_fn
...
def cmd_login(args):
    """Authenticate ShuoZi CLI with a provider."""
    from shuozi_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from shuozi_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from shuozi_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from shuozi_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from shuozi_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from shuozi_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``shuozi slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: shuozi slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `shuozi slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from shuozi_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from shuozi_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from shuozi_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from shuozi_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `shuozi security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from shuozi_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from shuozi_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from shuozi_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from shuozi_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from shuozi_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from shuozi_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from shuozi_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from shuozi_cli.banner import format_banner_version_label
    ...
    try:
        from shuozi_cli.banner import check_for_updates
        from shuozi_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall ShuoZi Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from shuozi_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from shuozi_cli.uninstall import run_gui_uninstall
    ...
    from shuozi_cli.uninstall import run_uninstall
...
# Critical files that every ``shuozi`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``shuozi update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "shuozi_cli/main.py",
    "shuozi_cli/config.py",
    "shuozi_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``shuozi`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``shuozi update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``shuozi_cli/web_dist/`` (per vite.config.ts
    outDir: "../shuozi_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "shuozi_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``shuozi`` launcher present but ``shuozi_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the shuozi venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — shuozi venv python3, no nix-shell overhead
    ...
    # a shuozi venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``shuozi update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``shuozi web``).
    ...
    # build path usable on a stock `py -m shuozi_cli.main web` invocation.
...
#   - ``shuozi update`` can unconditionally call ``shuozi desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``shuozi desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SHUOZI_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SHUOZI_HOME."""
    from shuozi_constants import get_shuozi_home
    return get_shuozi_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``shuozi update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "linux-unpacked" / "shuozi",
            release_dir / "linux-unpacked" / "Hermes",
            release_dir / "linux-arm64-unpacked" / "shuozi",
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from shuozi_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["SHUOZI_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["SHUOZI_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "hermes_root", None):
        env["SHUOZI_DESKTOP_SHUOZI_ROOT"] = str(Path(args.hermes_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SHUOZI_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  shuozi gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> shuozi desktop --force-build")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``shuozi dashboard`` processes other than ourselves.

    ``shuozi dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``shuozi update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``shuozi dashboard`` as
    ...
    environment variable ``SHUOZI_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "shuozi dashboard",
        "shuozi_cli.main dashboard",
        "shuozi_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "shuozi.*dashboard"`) keeps us consistent
            # with `shuozi_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `shuozi update`.
    ...
    print("  Preview now:  shuozi curator run --dry-run")
    print("  Pause it:     shuozi curator pause")
    print(
        "  Docs:         https://shuozi-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``shuozi update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``shuozi update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: shuozi curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``shuozi dashboard`` processes.

    Called at the end of ``shuozi update`` (default ``reason``) and also
    from ``shuozi dashboard --stop`` (which overrides ``reason``).  The
    ...
    # backend child, it sets SHUOZI_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SHUOZI_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    shuozi dashboard --port <port>")
...
def _update_via_zip(args):
    """Update ShuoZi Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `shuozi update "
            f"--branch {branch}`, or update against main with `shuozi update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/shuozi-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"shuozi-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for shuozi-agent itself
        ...
        # GitHub ZIPs extract to shuozi-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"shuozi-agent-{branch}")
    ...
    from shuozi_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from shuozi_cli.model_catalog import seed_cache_from_checkout
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes shuozi completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `shuozi update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/shuozi-agent.git",
    "git@github.com:NousResearch/shuozi-agent.git",
    "https://github.com/NousResearch/shuozi-agent",
    "git@github.com:NousResearch/shuozi-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/shuozi-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_shuozi_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_shuozi_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/shuozi-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/shuozi-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/shuozi-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``shuozi update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``shuozi update`` mutates the
...
# survives and the next ``shuozi`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $SHUOZI_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``shuozi update`` left half-done.
    ...
    protocol stream (``shuozi acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `shuozi update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from shuozi_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``shuozi update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    so during ``shuozi update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``shuozi update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``shuozi update``
...
def _format_concurrent_instances_message(
    ...
    lines.append("  stop the gateway (`shuozi gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `shuozi update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``shuozi update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    the next shuozi invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            ...
            "gateway, or pause AV scanning, then re-run `shuozi update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    is left missing and ``shuozi`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    Called early on every shuozi invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``shuozi update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `shuozi update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``shuozi update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``shuozi update``, not the venv we just wrote
    ...
    # live launcher, which would leave ``shuozi`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `shuozi update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `shuozi update --force` after closing other shuozi processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from shuozi_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SHUOZI_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from shuozi_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `shuozi update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``shuozi update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.shuozi/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``shuozi update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``shuozi update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.shuozi/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``shuozi update --gateway``) the update is already
    ...
    try:
        ...
        # shuozi_cli.config.get_shuozi_home to simulate setup failure.
        from shuozi_cli.config import get_shuozi_home as _get_shuozi_home

        logs_dir = _get_shuozi_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``shuozi update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from shuozi_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``shuozi update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from shuozi_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from shuozi_cli.config import recommended_update_command
        from shuozi_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``shuozi update`` without requiring a reinstall.  The
    ...
    ``shuozi`` prints ``command not found`` even though the symlink lives
    ...
    # /usr/local/bin/hermes, code at /usr/local/lib/shuozi-agent).
    ...
    path_comment = (
        "# ShuoZi Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SHUOZI_HOME before running the update.
    ...
    SHUOZI_HOME directories.  The ``--backup`` flag on ``shuozi update``
    ...
    try:
        from shuozi_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_shuozi_home so the user sees ~/.shuozi/...
    try:
        from shuozi_constants import get_shuozi_home, display_shuozi_home

        home = get_shuozi_home()
        try:
            display_path = f"{display_shuozi_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  shuozi import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``shuozi update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update ShuoZi Agent to the latest version.
    ...
    if is_managed():
        managed_error("update ShuoZi Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `shuozi update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    from shuozi_cli import __version__
    from shuozi_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "shuozi-agent"]
        ...
        cmd = [pipx, "upgrade", "shuozi-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "shuozi-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "shuozi-agent"]
    ...
    print("✓ Update complete! Restart shuozi to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://shuozi-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $SHUOZI_HOME, this is cheap
        ...
        try:
            from shuozi_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in shuozi_cli/config.py bricked
        # every user who ran ``shuozi update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``shuozi update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_shuozi_home added to hermes_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``shuozi`` launch finishes the
        ...
        # build.  ``shuozi desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``shuozi desktop`` shouldn't be forced into a full
        # Electron build by ``shuozi update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "shuozi_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `shuozi desktop` to retry)")
        ...
        # current — so copy it straight over ~/.shuozi/cache/model_catalog.json
        ...
        # attributes like display_shuozi_home() added since the last release.
        try:
            import importlib
            import shuozi_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SHUOZI_HOME so
        # it is not affected by sync_skills()'s module-level SHUOZI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SHUOZI_HOME env var points at the default or a named profile.
        try:
            from shuozi_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from shuozi_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'shuozi config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: shuozi config migrate")
            else:
                print()
                print("Skipped. Run 'shuozi config migrate' later to configure.")
        else:
        ...
        try:
            from shuozi_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `shuozi curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``shuozi update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from shuozi_cli.tools_config import install_cua_driver
        ...
        # When running as ``shuozi update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_shuozi_home() / ".update_exit_code"
            try:
        ...
        try:
            from shuozi_cli.gateway import (
                is_macos,
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `shuozi update` restart it automatically, allow\n"
                            ...
                            # path in `shuozi gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: shuozi gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: shuozi gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: shuozi -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `shuozi update` surfaces the issue until the user migrates.
        try:
            from shuozi_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    shuozi gateway migrate-legacy")
                print()
        ...
        print("  shuozi model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``shuozi -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from shuozi_constants import display_shuozi_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `shuozi profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_shuozi_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → shuozi -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.shuozi)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  shuozi profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     shuozi -p {name} chat")
                else:
        ...
        from shuozi_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from shuozi_constants import get_shuozi_home as _hh
        ...
        from shuozi_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `shuozi profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → shuozi -p {name}  ({wrapper})")
        print()
        ...
        from shuozi_cli.profiles import profile_exists
        ...
        from shuozi_cli.profiles import rename_profile
        ...
        from shuozi_cli.profiles import export_profile
        ...
        from shuozi_cli.profiles import import_profile
        ...
        import tempfile
        from shuozi_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  shuozi -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      shuozi -p {plan.manifest.name} chat")
        ...
        from shuozi_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `shuozi profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"shuozi -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from shuozi_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from shuozi_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``shuozi dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``shuozi dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No shuozi dashboard processes running.")
        return 0

    print(f"{len(pids)} shuozi dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No shuozi dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `shuozi update`.
    ...
    # launches the dashboard (`worker dashboard` → SHUOZI_HOME points into
    ...
    try:
        from shuozi_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("SHUOZI_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "shuozi_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        env = os.environ.copy()
        # Drop the profile SHUOZI_HOME so the child binds the machine root.
        env.pop("SHUOZI_HOME", None)
    ...
    if "SHUOZI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SHUOZI_WEB_DIST"])
            if "SHUOZI_WEB_DIST" in os.environ
            else PROJECT_ROOT / "shuozi_cli" / "web_dist"
        )
    ...
    try:
        from shuozi_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    try:
        from shuozi_cli.mcp_startup import start_background_mcp_discovery
    ...
    from shuozi_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from shuozi_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from shuozi_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from shuozi_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from shuozi_cli.logs import tail_log, list_logs
...
# so that in ``shuozi -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``shuozi_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``shuozi -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``shuozi`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SHUOZI_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from shuozi_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from shuozi_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SHUOZI_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SHUOZI_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SHUOZI_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `shuozi --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_shuozi_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_shuozi_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_shuozi_home()}/memories/\n")
    else:
        from shuozi_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch ShuoZi Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from shuozi_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from shuozi_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from shuozi_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from shuozi_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from shuozi_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from shuozi_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from shuozi_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from shuozi_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from shuozi_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for shuozi CLI."""
    # Cosmetic: make the process show up as 'shuozi' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from shuozi_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``hermes.exe.old.*`` quarantine files left by previous
    # ``shuozi update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # Self-heal a venv left half-built by an interrupted ``shuozi update``
    ...
    # ``shuozi skills install update``) merely defers recovery one launch;
    # under-matching (missing ``shuozi -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in shuozi_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from shuozi_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://shuozi-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `shuozi model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.shuozi/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://shuozi-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from shuozi_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from shuozi_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in shuozi_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in shuozi_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in shuozi_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in shuozi_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `shuozi whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in shuozi_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from shuozi_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in shuozi_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in shuozi_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in shuozi_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in shuozi_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in shuozi_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in shuozi_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from shuozi_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from shuozi_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in shuozi_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in shuozi_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in shuozi_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in shuozi_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in shuozi_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in shuozi_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.shuozi/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo shuozi uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from shuozi_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in shuozi_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in shuozi_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in shuozi_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in shuozi_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from shuozi_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in shuozi_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``shuozi --help``, ``shuozi version``, ``shuozi logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from shuozi_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from shuozi_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in shuozi_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in shuozi_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `shuozi computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `shuozi tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: shuozi computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: shuozi computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in shuozi_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_shuozi_home() / "sessions"
            ...
            # Launch shuozi --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in shuozi_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in shuozi_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in shuozi_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in shuozi_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in shuozi_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in shuozi_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in shuozi_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in shuozi_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # run `shuozi desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in shuozi_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in shuozi_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in shuozi_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``shuozi -c Pokemon Agent Dev`` → ``shuozi -c 'Pokemon Agent Dev'``
    ...
    from shuozi_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'shuozi -c model' where 'model' is consumed as the
