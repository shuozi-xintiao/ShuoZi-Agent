"""MCP picker — interactive `shuozi mcp picker` (also the default `shuozi mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``shuozi mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `shuozi plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from shuozi_cli.cli_output import prompt_yes_no
from shuozi_cli.curses_ui import curses_single_select
from shuozi_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from shuozi_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `shuozi mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: shuozi mcp install <name>    Picker: shuozi mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `shuozi update` "
...
def show_catalog() -> None:
    """`shuozi mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`shuozi mcp picker` (and default `shuozi mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`shuozi mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `shuozi mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
