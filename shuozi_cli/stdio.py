...
Entry points (``cli.py`` ``main``, ``shuozi_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SHUOZI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SHUOZI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
...
def _default_windows_editor() -> str:
    ...
    ``shuozi config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "shuozi", "git", "cmd"),
        os.path.join(local_appdata, "shuozi", "git", "bin"),
        os.path.join(local_appdata, "shuozi", "git", "usr", "bin"),
        ...
        os.path.join(local_appdata, "shuozi", "shuozi-agent", "venv", "Scripts"),
