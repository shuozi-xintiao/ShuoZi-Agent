...
``shuozi -p <profile>`` joins the same board as the dispatcher that
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SHUOZI_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SHUOZI_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``shuozi kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.shuozi``. In Docker / custom
deployments where ``SHUOZI_HOME`` points outside ``~/.shuozi`` (e.g.
``/opt/hermes``), ``<root>`` is ``SHUOZI_HOME``. Legacy env-var
overrides still work:

* ``SHUOZI_KANBAN_DB`` — pin the database file path directly.
* ``SHUOZI_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SHUOZI_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SHUOZI_KANBAN_DB``,
``SHUOZI_KANBAN_WORKSPACES_ROOT``, and ``SHUOZI_KANBAN_BOARD`` into
...
# ``SHUOZI_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``SHUOZI_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("SHUOZI_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``SHUOZI_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("SHUOZI_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``SHUOZI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "SHUOZI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``shuozi-agent``
...
def kanban_home() -> Path:
    ...
    1. ``SHUOZI_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``SHUOZI_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SHUOZI_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SHUOZI_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("SHUOZI_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``shuozi kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SHUOZI_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``shuozi kanban boards
    ...
    env = os.environ.get("SHUOZI_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``shuozi kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SHUOZI_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SHUOZI_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SHUOZI_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("SHUOZI_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``SHUOZI_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("SHUOZI_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``shuozi kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``SHUOZI_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``SHUOZI_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("SHUOZI_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SHUOZI_KANBAN_DB`` env → ``SHUOZI_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See shuozi_state._WAL_INCOMPAT_MARKERS for detection logic.
                from shuozi_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``shuozi kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``SHUOZI_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from shuozi_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``shuozi --skills ...`` alongside the built-in
    ...
    # `shuozi --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``shuozi kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `shuozi kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``shuozi kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``shuozi kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``shuozi kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``shuozi kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``SHUOZI_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
#      via ``shuozi kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``SHUOZI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``shuozi kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from shuozi_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from shuozi_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        # `_default_spawn` invokes ``shuozi -p <assignee>`` which fails
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `shuozi kanban tail` — without
...
def _resolve_hermes_argv() -> list[str]:
    ...
    1. ``$SHUOZI_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("shuozi")`` — the console-script shim, normalized to
    ...
    local (not imported from gateway) because ``shuozi_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("SHUOZI_BIN", "").strip()
    if env_bin:
    ...
    hermes_bin = _safe_which_no_cwd("shuozi") if _IS_WINDOWS else shutil.which("shuozi")
    if hermes_bin:
        return _hermes_path_argv(hermes_bin)
    return _module_hermes_argv()


def _kanban_worker_skill_available(shuozi_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``shuozi -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SHUOZI_HOME means the worker falls back to the default root
    # home (``~/.shuozi``), which ships the bundled skill.
    base = _Path(shuozi_home) if shuozi_home else (_Path.home() / ".hermes")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``shuozi -p <profile> chat -q ...`` subprocess.
    ...
    ``SHUOZI_KANBAN_DB`` / ``SHUOZI_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject SHUOZI_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `shuozi -p <name>` the
    # _apply_profile_override() runs *before* hermes_constants is imported.
    # If SHUOZI_HOME is absent from the child's env, get_shuozi_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from shuozi_cli.profiles import resolve_profile_env
    try:
        env["SHUOZI_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via SHUOZI_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SHUOZI_HOME never had profiles created.
        pass
    if task.tenant:
        env["SHUOZI_TENANT"] = task.tenant
    env["SHUOZI_KANBAN_TASK"] = task.id
    env["SHUOZI_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["SHUOZI_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["SHUOZI_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SHUOZI_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["SHUOZI_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["SHUOZI_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`shuozi -p <name>`
    # rewrites SHUOZI_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["SHUOZI_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["SHUOZI_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SHUOZI_KANBAN_BOARD"] = resolved_board
    # SHUOZI_PROFILE is the author the kanban_comment tool defaults to.
    # `shuozi -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SHUOZI_PROFILE"] = profile_arg

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped SHUOZI_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("SHUOZI_HOME")):
    ...
    # `shuozi kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install ShuoZi Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``shuozi kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled SHUOZI_PROFILE values like "hermes-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``shuozi_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from shuozi_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``shuozi kanban assignees`` for the terminal.
