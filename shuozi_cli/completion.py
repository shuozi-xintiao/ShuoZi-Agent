"""Shell completion script generation for shuozi CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# ShuoZi Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hermes completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.shuozi/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.shuozi/profiles" ]]; then
        profiles+=($HOME/.shuozi/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'shuozi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# ShuoZi Agent fish completion",
        "# Add to your config:",
        "#   shuozi completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.shuozi/profiles",
        "        for d in $HOME/.shuozi/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c shuozi -f -s p -l profile"
