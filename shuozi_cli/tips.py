...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or shuozi -c.",
    ...
    # --- CLI Flags ---
    "shuozi -c resumes your most recent CLI session. shuozi -c \"project name\" resumes by title.",
    "shuozi -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "shuozi -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "shuozi chat -t web,terminal enables only specific toolsets for a focused session.",
    "shuozi chat -s github-pr-workflow preloads a skill at launch.",
    "shuozi chat -q \"query\" runs a single non-interactive query and exits.",
    "shuozi chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "shuozi chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "shuozi --yolo bypasses all dangerous command approval prompts for the entire session.",
    "shuozi chat --source telegram tags the session for filtering in shuozi sessions list.",
    "shuozi -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "shuozi doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "shuozi dump outputs a compact setup summary — great for bug reports.",
    "shuozi config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "shuozi config edit opens config.yaml in your default editor.",
    "shuozi config check scans for missing or stale configuration options.",
    "shuozi sessions browse opens an interactive session picker with search.",
    "shuozi sessions stats shows session counts by platform and database size.",
    "shuozi sessions prune --older-than 30 cleans up old sessions.",
    "shuozi skills search react --source skills-sh searches the skills.sh public directory.",
    "shuozi skills check scans installed hub skills for upstream updates.",
    "shuozi skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "shuozi skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "shuozi mcp add github --command npx adds MCP servers from the command line.",
    "shuozi mcp serve runs Hermes itself as an MCP server for other agents.",
    "shuozi auth add lets you add multiple API keys for credential pool rotation.",
    "shuozi completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "shuozi logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "shuozi backup creates a zip backup of your entire Hermes home directory.",
    "shuozi profile create coder creates an isolated profile that becomes its own command.",
    "shuozi profile create work --clone copies your current config and keys to a new profile.",
    "shuozi update syncs new bundled skills to ALL profiles automatically.",
    "shuozi gateway install sets up Hermes as a system service (systemd/launchd).",
    "shuozi memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "shuozi webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: shuozi tools disables unused tools, shuozi skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "shuozi models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as shuozi -w).",
    ...
    "SHUOZI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'shuozi profile create coder' creates the 'coder' command.",
    "shuozi profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "shuozi sessions export backup.jsonl exports all sessions for backup or analysis.",
    "shuozi -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "shuozi skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .shuozi/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: shuozi cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "shuozi gateway install sets it up as a system service that starts on boot.",
    ...
    # --- MCP ---
    "shuozi mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "shuozi mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "shuozi mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — shuozi tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.shuozi/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "shuozi chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "shuozi plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "shuozi acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SHUOZI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "shuozi auth add nous or shuozi auth add openai-codex sets up OAuth-based providers.",
    ...
    "shuozi status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.shuozi/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.shuozi/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SHUOZI_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SHUOZI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SHUOZI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.shuozi/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'shuozi profile create ls' would be rejected.",
    "shuozi profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "shuozi claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SHUOZI_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via shuozi tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.shuozi/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.shuozi/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.shuozi/.env into the running session — pick up new API keys without restarting.',
    ...
    "SHUOZI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.shuozi/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.shuozi/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'shuozi curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "shuozi curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'shuozi curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'shuozi auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SHUOZI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SHUOZI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'shuozi dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'shuozi dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.shuozi/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.shuozi/dashboard-plugins/ — no npm build required.',
    ...
    'SHUOZI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SHUOZI_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SHUOZI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SHUOZI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.shuozi/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.shuozi/skills/ so drop-in skills appear without restarting the session.',
    ...
    'shuozi -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'shuozi chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'shuozi chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'shuozi chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "shuozi chat --source tool tags programmatic chats so they don't clutter shuozi sessions list.",
    'shuozi dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'shuozi sessions rename <ID> "new title" renames any past session; shuozi sessions delete <ID> removes one.',
    'shuozi import restores a session export or profile archive produced by sessions export or profile export.',
    'shuozi fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'shuozi pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'shuozi setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'shuozi status --deep runs the full health sweep across every component; plain shuozi status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SHUOZI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SHUOZI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.shuozi/plugins/ — trust-gated by design.',
    "SHUOZI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SHUOZI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SHUOZI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SHUOZI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SHUOZI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SHUOZI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SHUOZI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SHUOZI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SHUOZI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SHUOZI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SHUOZI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    '--source tool chats are excluded from shuozi sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.shuozi/dashboard-plugins/.',
]
