...
from shuozi_cli._parser import (
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the shuozi entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("shuozi")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m shuozi_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m shuozi_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "shuozi_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh shuozi invocation.
    ...
    relaunch contract wants: "run shuozi again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m shuozi_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "shuozi exited, then
    new shuozi started" — just with two PIDs in play instead of one.
    """
