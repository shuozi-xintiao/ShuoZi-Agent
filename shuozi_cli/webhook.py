"""shuozi webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    shuozi webhook subscribe <name> [options]
    shuozi webhook list
    shuozi webhook remove <name>
    shuozi webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.shuozi/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
    _dhh = display_shuozi_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     shuozi gateway setup
  ...
  Then start the gateway: shuozi gateway run
"""
...
def webhook_command(args):
    """Entry point for 'shuozi webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: shuozi webhook {subscribe|list|remove|test}")
        print("Run 'shuozi webhook --help' for details.")
        return
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: shuozi webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from shuozi webhook test"}'
