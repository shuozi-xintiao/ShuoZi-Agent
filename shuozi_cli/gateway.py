"""
Gateway subcommand for shuozi CLI.

Handles: shuozi gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_shuozi_home is imported lazily at call sites to avoid ImportError
# when hermes_constants is cached from a pre-update version during `shuozi update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "shuozi CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``shuozi gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "shuozi_cli.main gateway",
        "shuozi_cli.main --profile",
        "shuozi_cli.main -p",
        "shuozi_cli/main.py gateway",
        "shuozi_cli/main.py --profile",
        "shuozi_cli/main.py -p",
        "shuozi gateway",
    ...
    current_home = str(get_shuozi_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"shuozi_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SHUOZI_HOME
        ...
        # SHUOZI_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "shuozi_home=" in command_lc
            and f"shuozi_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``shuozi update``
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from shuozi_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "shuozi_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``shuozi update`` users
    # with no running gateway until they re-invoke ``shuozi gateway``
    ...
    from shuozi_cli._subprocess_compat import (
...
def _sync_shuozi_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SHUOZI_HOME``.

    Under ``sudo``, ``SHUOZI_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_shuozi_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``SHUOZI_HOME`` for the actual gateway
    ...
    unit_home = env.get("SHUOZI_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SHUOZI_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SHUOZI_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from shuozi_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `shuozi gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``shuozi gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from shuozi_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SHUOZI_HOME-scoped).
...
from shuozi_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``shuozi gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SHUOZI_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SHUOZI_GATEWAY_DETACHED", "").strip().lower()
...
SERVICE_DESCRIPTION = "ShuoZi Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SHUOZI_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SHUOZI_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from shuozi_constants import get_default_hermes_root

    home = get_shuozi_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SHUOZI_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(shuozi_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when SHUOZI_HOME is a named profile.

    For ``~/.shuozi/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        shuozi_home: Optional explicit SHUOZI_HOME path. Defaults to the current
            ``get_shuozi_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from shuozi_constants import get_default_hermes_root

    home = Path(shuozi_home or str(get_shuozi_home())).resolve()
    default = get_default_hermes_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SHUOZI_HOME.

    Default ``~/.shuozi`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.shuozi/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other SHUOZI_HOME appends a short hash for uniqueness.
    """
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    shuozi gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: shuozi gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo shuozi gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    shuozi gateway uninstall")
    print_info("    sudo shuozi gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so ShuoZi can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo shuozi gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo shuozi gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo shuozi gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.shuozi`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.shuozi/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.shuozi/shuozi-agent  -> /home/alice/.shuozi/shuozi-agent
...
def _shuozi_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SHUOZI_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_shuozi_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.shuozi/profiles/coder     → /home/alice/.shuozi/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_shuozi_home().resolve()
    current_default = (Path.home() / ".hermes").resolve()
    target_default = Path(target_home_dir) / ".hermes"

    # Default ~/.shuozi → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.shuozi → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.shuozi) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    shuozi_home = get_shuozi_home()
    hermes_node = shuozi_home / "node" / "bin"
    if _is_dir(hermes_node):
        candidates.append(str(hermes_node))
    hermes_nm = shuozi_home / "node_modules" / ".bin"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m shuozi_cli.main``, so module
    ...
    dir, or a clone that ``shuozi update`` later relocates/removes — the path
    ...
    ``SHUOZI_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if SHUOZI_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        shuozi_home = _shuozi_home_for_target_user(home_dir)
        profile_arg = _profile_arg(shuozi_home)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's SHUOZI_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(shuozi_home) if shuozi_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m shuozi_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="SHUOZI_HOME={shuozi_home}"
Restart=always
RestartSec=5
...
"""
    ...
    shuozi_home = str(get_shuozi_home().resolve())
    profile_arg = _profile_arg(shuozi_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir SHUOZI_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved SHUOZI_HOME
    ...
    exported a throwaway ``SHUOZI_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``shuozi gateway restart``
    with ``SHUOZI_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="SHUOZI_HOME=..."`` lines and launchd
    ``<key>SHUOZI_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'SHUOZI_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>SHUOZI_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp SHUOZI_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: SHUOZI_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported SHUOZI_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only SHUOZI_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SHUOZI_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="SHUOZI_HOME=..."``. Writing that to the developer's
    ...
    # SHUOZI_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo shuozi gateway uninstall --system")
    print_info("         shuozi gateway install")
    print_info("         shuozi gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("SHUOZI_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_stop(system: bool = False):
    ...
    _sync_shuozi_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `shuozi gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `shuozi gateway status` or logs for final state."
            )
            return
...
# process invocation (each ``shuozi gateway start/stop/status`` call).
...
# (the documented `nohup shuozi gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m shuozi_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active SHUOZI_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "shuozi_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup shuozi gateway run --replace`
    ...
    """
    from shuozi_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_shuozi_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from shuozi_constants import display_shuozi_home as _dhh
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: shuozi gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup shuozi gateway run --replace "
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  shuozi gateway status             # Check status")
    from shuozi_constants import display_shuozi_home as _dhh
...
def launchd_stop():
    ...
    # `shuozi gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: shuozi gateway start")
    ...
    if deep:
        log_file = get_shuozi_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SHUOZI_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'shuozi' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Hermes command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SHUOZI_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set SHUOZI_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `shuozi gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SHUOZI_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `shuozi gateway restart` which already
    ...
    # `shuozi gateway start/restart` — leaving the gateway vulnerable to
    ...
    # SHUOZI_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SHUOZI_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from shuozi_constants import get_shuozi_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your ShuoZi Agent",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: shuozi pairing generate bluebubbles",
...
def _all_platforms() -> list[dict]:
    ...
    ``shuozi setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run shuozi under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.shuozi/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_shuozi_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from shuozi_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'shuozi pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: shuozi pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'shuozi gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from shuozi_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from shuozi_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: shuozi pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'shuozi gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from shuozi_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.shuozi/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `shuozi gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_shuozi_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `shuozi pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `shuozi pairing approve`."
        )
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``shuozi_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from shuozi_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.shuozi/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.shuozi/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: shuozi gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: shuozi gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: shuozi gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo shuozi gateway install --system"
                        )
                    print_info("  Or run in foreground:  shuozi gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: shuozi gateway run")
                print_info(
                    "  For persistence:   tmux new -s shuozi 'shuozi gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: shuozi gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup shuozi gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'shuozi gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`shuozi_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from shuozi_cli.service_manager import (
    ...
    if profile is None:
        ...
        # SHUOZI_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``shuozi gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``shuozi gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``shuozi gateway run`` are unaffected.
      2. ``SHUOZI_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``shuozi gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``SHUOZI_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("SHUOZI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("SHUOZI_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${SHUOZI_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if SHUOZI_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or SHUOZI_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: shuozi gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: shuozi gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s shuozi 'shuozi gateway run'"
                )
                print()
            ...
            print()
            print(
                "  shuozi gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s shuozi 'shuozi gateway run'         # persistent via tmux"
            )
            print(
                "  nohup shuozi gateway run > ~/.shuozi/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  shuozi profile create <name>     # creates the s6 service slot")
                print("  shuozi -p <name> gateway start   # bring it up via s6")
                print("  shuozi status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: shuozi gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: shuozi gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: shuozi gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  shuozi profile delete <name>     # tears down the s6 service slot")
                print("  shuozi -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `shuozi -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: shuozi gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_SHUOZI_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `shuozi gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_SHUOZI_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `shuozi gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: shuozi gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  shuozi gateway install")
                else:
                    print("To install as a service:")
                    print("  shuozi gateway install")
                    print("  sudo shuozi gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  shuozi gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup shuozi gateway run > ~/.shuozi/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s shuozi 'shuozi gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  shuozi gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  shuozi gateway install  # Install as user service")
                    print(
                        "  sudo shuozi gateway install --system  # Install as boot-time system service"
                    )
