"""
Doctor command for shuozi CLI.

Diagnoses issues with ShuoZi Agent setup.
"""
...
from shuozi_cli.config import get_project_root, get_shuozi_home, get_env_path
...
_DHH = display_shuozi_home()  # user-facing display path (e.g. ~/.shuozi or ~/.shuozi/profiles/coder)

# Load environment variables from ~/.shuozi/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(shuozi_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from shuozi_cli.colors import Colors, color
from shuozi_cli.models import _SHUOZI_USER_AGENT
from shuozi_constants import OPENROUTER_MODELS_URL
...
from shuozi_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.shuozi/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SHUOZI_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from shuozi_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from shuozi_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches shuozi_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``shuozi --version`` reporting a stale version while
    ...
    """
    try:
        from shuozi_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != shuozi_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'shuozi update', or set "
            "shuozi_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `shuozi profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from shuozi_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    os.environ.setdefault("SHUOZI_INTERACTIVE", "1")

    # Handle `shuozi doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.shuozi/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`shuozi doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and shuozi_cli/__init__.py versions
    ...
    # Check ~/.shuozi/.env (primary location for user config)
    env_path = SHUOZI_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'shuozi setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'shuozi setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'shuozi setup' to create one")
                issues.append("Run 'shuozi setup' to create .env")
    
    # Check ~/.shuozi/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = SHUOZI_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from shuozi_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from shuozi_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'shuozi config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from shuozi_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.shuozi/.env or run 'shuozi setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'shuozi setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'shuozi config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from shuozi_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from shuozi_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'shuozi setup' to migrate config")
                else:
                    issues.append("Run 'shuozi doctor --fix' or 'shuozi setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'shuozi doctor --fix'")
        except Exception:
            pass

        # Detect stale SHUOZI_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives SHUOZI_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from shuozi_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("SHUOZI_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"SHUOZI_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `shuozi setup` run)",
                )
                if should_fix:
                    if remove_env_value("SHUOZI_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale SHUOZI_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove SHUOZI_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the SHUOZI_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale SHUOZI_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'shuozi doctor --fix'"
                    )
        except Exception:
            pass
    ...
    if shuozi_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        shuozi_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = shuozi_home / subdir_name
    ...
    memories_dir = shuozi_home / "memories"
    ...
    state_db_path = shuozi_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from shuozi_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'shuozi doctor --fix' "
                        "(or 'shuozi sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = shuozi_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'shuozi doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "shuozi"
        ...
        _cmd_link = _cmd_link_dir / "shuozi"
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    ...
                    if should_fix:
                        ...
                        issues.append(f"Broken symlink at {_cmd_link_display}/hermes — run 'shuozi doctor --fix'")
                ...
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/hermes symlink — run 'shuozi doctor --fix'")
    ...
    try:
        from shuozi_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `shuozi doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'shuozi config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``shuozi doctor``.
    ...
    # never the legitimate source for `shuozi doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'shuozi setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = SHUOZI_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: shuozi skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = SHUOZI_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: shuozi memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: shuozi memory setup",
                    "No Honcho API key — run 'shuozi memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run shuozi memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: shuozi memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: shuozi memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from shuozi_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "shuozi -p" in content:
                            _m = _re.search(r"shuozi -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'shuozi doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
