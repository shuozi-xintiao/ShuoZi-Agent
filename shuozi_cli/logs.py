"""``shuozi logs`` — view and filter Hermes log files.
...
Usage examples::
    ...
    shuozi logs                    # last 50 lines of agent.log
    shuozi logs -f                 # follow agent.log in real time
    shuozi logs errors             # last 50 lines of errors.log
    shuozi logs gateway -n 100    # last 100 lines of gateway.log
    shuozi logs gui -f            # follow gui.log (dashboard/pty/ws)
    shuozi logs desktop -f        # follow desktop.log (Electron app boot/backend)
    shuozi logs --level WARNING    # only WARNING+ lines
    shuozi logs --session abc123   # filter by session ID substring
    shuozi logs --component tools  # only tool-related lines
    shuozi logs --since 1h         # lines from the last hour
    shuozi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shuozi_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Hermes runs — try 'shuozi chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_shuozi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_shuozi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shuozi_home()}/logs/")
        return

    print(f"Log files in {display_shuozi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shuozi chat' to generate logs)")
