...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_shuozi_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:shuozi_pkce",
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `shuozi --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``shuozi auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`shuozi auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `shuozi auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `shuozi auth` is called bare."""
