"""Windows ConPTY bridge for the `shuozi dashboard` chat tab.

Drop-in counterpart to ``shuozi_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``shuozi_cli.web_server`` consumes: ``spawn``, ``read``,
