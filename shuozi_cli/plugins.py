...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with shuozi-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.shuozi/plugins/<name>/``
3. **Project plugins** – ``./.shuozi/plugins/<name>/`` (opt-in via
   ``SHUOZI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``shuozi_agent.plugins``
   entry-point group.
...
from shuozi_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``SHUOZI_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SHUOZI_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SHUOZI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.shuozi/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SHUOZI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SHUOZI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "SHUOZI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "shuozi_agent.plugins"
...
class PluginManifest:
    ...
    #              in ~/.shuozi/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``shuozi plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``shuozi honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``shuozi <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`shuozi_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from shuozi_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``shuozi model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: shuozi_cli.main imports plugins indirectly
        from shuozi_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.shuozi/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to shuozi_cli/. Two layouts are
        ...
        # 2. User plugins (~/.shuozi/plugins/)
        user_dir = get_shuozi_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.shuozi/plugins/)
        if _env_enabled("SHUOZI_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set SHUOZI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with shuozi and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `shuozi plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `shuozi plugins list`.
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``shuozi tools`` TUI so plugin-provided toolsets appear
