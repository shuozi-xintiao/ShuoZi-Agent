...
from shuozi_cli.config import get_shuozi_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``SHUOZI_GIT_SHA`` build-arg (see ``shuozi_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``shuozi dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from shuozi_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(shuozi_home: Path) -> int:
...
def _cron_summary(shuozi_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = shuozi_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"shuozi_home:      {display_shuozi_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `shuozi auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `shuozi auth list` shows it (#42130).
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(shuozi_home)}")
    lines.append(f"  skills:             {_count_skills(shuozi_home)}")
