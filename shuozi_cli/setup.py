"""
Interactive setup wizard for ShuoZi Agent.
...
_DOCS_BASE = "https://shuozi-agent.nousresearch.com/docs"
...
# display_shuozi_home imported lazily at call sites (stale-module safety during shuozi update)
...
from shuozi_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from shuozi_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  shuozi config set model.provider custom")
    print_info("  shuozi config set model.base_url http://localhost:8080/v1")
    print_info("  shuozi config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'shuozi setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from shuozi_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'shuozi setup')")


def _print_setup_summary(config: dict, shuozi_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'shuozi setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from shuozi_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `shuozi tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from shuozi_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'shuozi setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'shuozi setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'shuozi setup terminal'"))
    ...
    # Spotify (OAuth via shuozi auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'shuozi setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {shuozi_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('shuozi setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('shuozi setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('shuozi setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('shuozi setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('shuozi setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('shuozi config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('shuozi config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('shuozi config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('shuozi', Colors.GREEN)}              Start chatting")
    print(f"   {color('shuozi gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('shuozi doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `shuozi tools` and `shuozi setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``shuozi model``)
    ...
    provider added to ``shuozi model`` is automatically available here.
    ...
    # Delegate to the shared shuozi model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from shuozi_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: shuozi model")
    ...
    # on demand via `shuozi auth add`, `shuozi setup` vision, and
    # `shuozi setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``shuozi model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from shuozi_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.shuozi/.env."
            )
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via shuozi setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'shuozi setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `shuozi setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `shuozi setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SHUOZI_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SHUOZI_MAX_ITERATIONS")
    ...
    print_info("  Run `shuozi setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SHUOZI_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from shuozi_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in shuozi_home:
            profile_name = shuozi_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `shuozi update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://shuozi-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under SHUOZI_HOME, and print
    ...
    """
    try:
        from shuozi_cli.slack_cli import _build_full_manifest
        from shuozi_constants import get_shuozi_home

        manifest = _build_full_manifest(
            bot_name="Hermes",
            bot_description="Your ShuoZi Agent on Slack",
        )
        target = Path(get_shuozi_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `shuozi slack manifest --write` anytime to refresh after "
...
def _setup_qqbot():
    ...
    from shuozi_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://shuozi-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://shuozi-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  shuozi config edit")
    print_info("   Open config in your editor:  shuozi config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from shuozi_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'shuozi setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     shuozi config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo shuozi gateway install --system")
                print_info("  Or run in foreground:  shuozi gateway")
        else:
            from shuozi_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   shuozi gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   shuozi gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `shuozi setup tools` and `shuozi tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``shuozi_cli.auth`` — lists every supported
    ...
    """
    try:
        from shuozi_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from shuozi_cli.config
    ...
    if section_key == "model":
        ...
        from shuozi_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(shuozi_home: Path) -> bool:
    ...
    print_info("ShuoZi can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: shuozi claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=shuozi_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: shuozi claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use shuozi claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``shuozi setup --portal`` and ``shuozi portal``. This is the
    ...
    ``shuozi setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``shuozi model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``shuozi portal`` always offers a model picker,
    ...
    # setup calls, so `shuozi portal` == quick setup's Nous step.
    try:
        from shuozi_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `shuozi portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `shuozi portal`: %s", exc)
        print()
    ...
    print_info("  Run `shuozi portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      shuozi setup           — full or quick (auto-detected)
      shuozi setup model     — just model/provider
      shuozi setup tts       — just text-to-speech
      shuozi setup terminal  — just terminal backend
      shuozi setup gateway   — just messaging platforms
      shuozi setup tools     — just tool configuration
      shuozi setup agent     — just agent settings
    """
    from shuozi_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│             ⚕ ShuoZi Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your ShuoZi Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, shuozi_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'shuozi setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(shuozi_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, shuozi_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {shuozi_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'shuozi config edit'")
    ...
    # Tune later with `shuozi setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, shuozi_home)


def _run_first_time_quick_setup(config: dict, shuozi_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``shuozi setup <section>``
    or switch providers with ``shuozi model``.
    """
    from shuozi_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'shuozi setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    shuozi setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  shuozi setup gateway")
    print()

    _print_setup_summary(config, shuozi_home)


def _run_quick_setup(config: dict, shuozi_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'shuozi setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'shuozi setup gateway'.")
