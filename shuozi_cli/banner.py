...
from shuozi_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built shuozi — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/shuozi-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/shuozi-agent"
...
def _fetch_pypi_latest(package: str = "shuozi-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``SHUOZI_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = shuozi_home / ".update_check"
    embedded_rev = os.environ.get("SHUOZI_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SHUOZI_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `shuozi update` correctly refuses to run in-place inside the
    ...
    if embedded_rev:
        ...
        # $SHUOZI_HOME/shuozi-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = shuozi_home / "shuozi-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active ShuoZi git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SHUOZI_HOME/shuozi-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``shuozi_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from shuozi_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/shuozi-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/shuozi-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"ShuoZi Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SHUOZI_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SHUOZI_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from shuozi_cli.codex_runtime_switch import get_current_runtime
        from shuozi_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from shuozi_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install shuozi-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SHUOZI_AGENT_LOGO
        console.print(_logo)
        console.print()
