"""``shuozi portal`` — the human-readable entry point for Nous Portal.

Running ``shuozi portal`` with no subcommand performs the one-shot Portal
...
``shuozi auth add nous --type oauth`` (which still works), is identical to
``shuozi setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``shuozi auth`` or ``shuozi tools``. It's the onboarding + discovery
...
DOCS_URL = "https://shuozi-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `shuozi model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `shuozi portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `shuozi auth add nous --type
    oauth`. It reuses the exact wiring behind `shuozi setup --portal` (which in
    ...
    """
    from shuozi_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `shuozi portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `shuozi portal` is the
        # human-readable alias for `shuozi auth add nous --type oauth` /
        # `shuozi setup --portal`.
    ...
    print("Run `shuozi portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `shuozi portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `shuozi portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `shuozi auth add "
            "nous --type oauth`, identical to `shuozi setup --portal`). "
