"""shuozi hooks — inspect and manage shell-script hooks.

Usage::

    shuozi hooks list
    shuozi hooks test <event> [--for-tool X] [--payload-file F]
    shuozi hooks revoke <command>
    shuozi hooks doctor

Consent records live under ``~/.shuozi/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.shuozi/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``shuozi hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: shuozi hooks {list|test|revoke|doctor}")
        print("Run 'shuozi hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.shuozi/config.yaml.")
        print("See `shuozi hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `shuozi hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `shuozi hooks test` and `shuozi hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `shuozi hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `shuozi hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `shuozi hooks doctor`.")
