...
MIGRATION_MARKER = (
    "# managed by shuozi-agent — `shuozi codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end shuozi-agent managed section"
)
...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_SHUOZI_KEYS:
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SHUOZI_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SHUOZI_HOME pointing
    ...
    ``~/.shuozi`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    so a shuozi installed under /opt/, /usr/local/, or a venv all work.
    SHUOZI_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SHUOZI_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_shuozi_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SHUOZI_HOME its launcher sets
    ...
    # — that would override the launcher's SHUOZI_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SHUOZI_HOME", tmp_path) would
    ...
    shuozi_home = os.environ.get("SHUOZI_HOME") or ""
    if shuozi_home and _looks_like_test_tempdir(shuozi_home):
        shuozi_home = ""
    if shuozi_home:
        env["SHUOZI_HOME"] = shuozi_home
    # PYTHONPATH passes through so a worktree-launched shuozi finds the
    ...
    env["SHUOZI_QUIET"] = "1"
    env["SHUOZI_REDACT_SECRETS"] = env.get("SHUOZI_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate ShuoZi mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.shuozi/config.yaml dict
