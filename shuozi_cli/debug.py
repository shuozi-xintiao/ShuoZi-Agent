"""``shuozi debug`` debug tools for ShuoZi Agent.

Currently supports:
    shuozi debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.shuozi/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.shuozi/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``shuozi debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``shuozi debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `shuozi debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "shuozi-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``shuozi debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.shuozi/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``shuozi debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "shuozi-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from shuozi_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_shuozi_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``shuozi dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``shuozi dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``shuozi debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "shuozi debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `shuozi debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  shuozi debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: shuozi debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'shuozi debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``shuozi debug`` call.
    ...
    # best-effort — any failure is swallowed so ``shuozi debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: shuozi debug <command>")
        print()
        print("Commands:")
