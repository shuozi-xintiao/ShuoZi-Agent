...
Each profile is a fully independent SHUOZI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.shuozi/profiles/<name>/`` by default.

The "default" profile is ``~/.shuozi`` itself — backward compatible,
zero migration needed.

Usage::

    shuozi profile create coder          # fresh profile + bundled skills
    shuozi profile create coder --clone  # also copy config, .env, SOUL.md, skills
    shuozi profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    shuozi -p coder chat                 # or via flag
    shuozi profile use coder             # set as sticky default
    shuozi profile delete coder          # remove profile + alias + service
"""
...
# default profile (``~/.shuozi``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   shuozi-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `shuozi backup` archives
...
# Marker file written by `shuozi profile create --no-skills`.  When present in
...
# `shuozi skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.shuozi``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_shuozi_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.shuozi) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "shuozi-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "shuozi", "default", "test", "tmp", "root", "sudo",
})

# Hermes subcommands that cannot be used as profile names/aliases
_SHUOZI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the shuozi root, NOT to the current SHUOZI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SHUOZI_HOME points outside
    ``~/.shuozi``, profiles live under ``SHUOZI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_shuozi_home() / "profiles"


def _get_default_shuozi_home() -> Path:
    """Return the default (pre-profile) SHUOZI_HOME path.

    In standard deployments this is ``~/.shuozi``.
    In Docker/custom deployments where SHUOZI_HOME is outside ``~/.shuozi``
    (e.g. ``/opt/data``), returns SHUOZI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_shuozi_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    """
    if name == "default":
        return  # special alias for ~/.shuozi
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SHUOZI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_shuozi_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, shuozi subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _SHUOZI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a shuozi subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``shuozi -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``shuozi profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``shuozi profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_shuozi_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``shuozi update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.shuozi)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.shuozi/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `shuozi -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `shuozi update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`shuozi profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `shuozi profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `shuozi -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SHUOZI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``shuozi profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHUOZI_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_shuozi_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.shuozi).\n"
            "To remove everything, use: shuozi uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from shuozi_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SHUOZI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SHUOZI_HOME")
    try:
        os.environ["SHUOZI_HOME"] = str(profile_dir)
        from shuozi_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SHUOZI_HOME"] = old_home
        elif "SHUOZI_HOME" in os.environ:
            del os.environ["SHUOZI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.shuozi/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: shuozi profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SHUOZI_HOME.

    Returns ``"default"`` if SHUOZI_HOME is not set or points to ``~/.shuozi``.
    Returns the profile name if SHUOZI_HOME points into ``~/.shuozi/profiles/<name>``.
    Returns ``"custom"`` if SHUOZI_HOME is set to an unrecognized path.
    """
    from shuozi_constants import get_shuozi_home
    shuozi_home = get_shuozi_home()
    resolved = shuozi_home.resolve()

    default_resolved = _get_default_shuozi_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.shuozi itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: shuozi profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.shuozi itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.shuozi). "
            "Specify a different name: shuozi profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_shuozi_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SHUOZI_HOME path string.

    Called early in the CLI entry point, before any shuozi modules
    are imported, to set the SHUOZI_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
