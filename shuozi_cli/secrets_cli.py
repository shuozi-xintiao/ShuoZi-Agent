"""CLI handlers for ``shuozi secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``shuozi_cli.main`` as part of building the top-level
    ``shuozi secrets`` parser.
    """
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`shuozi secrets bitwarden setup` first.[/yellow]"
        )
        return 1
