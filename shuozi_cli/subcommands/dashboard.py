"""``shuozi dashboard`` subcommand parser.

Extracted verbatim from ``shuozi_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the ShuoZi Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `shuozi dashboard` cmdlines and SIGTERM them directly — the same
    # path `shuozi update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running shuozi dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running shuozi dashboard processes and exit",
    )
    # Backward-compat shim: older Hermes desktop app shells (<= 0.15.x) spawn the
    # backend as `shuozi dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `shuozi dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.shuozi/.env.
    # Nested subparser so bare `shuozi dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "SHUOZI_DASHBOARD_OAUTH_CLIENT_ID into ~/.shuozi/.env, and prints "
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via SHUOZI_DASHBOARD_PORTAL_URL. Mainly for "
