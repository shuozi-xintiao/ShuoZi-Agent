"""``shuozi mcp`` subcommand parser.

Extracted from ``shuozi_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from shuozi_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'shuozi mcp add' to connect to a new server, or\n"
            "'shuozi mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `shuozi mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `shuozi mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `shuozi mcp install n8n`)",
    )
