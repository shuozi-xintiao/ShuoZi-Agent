"""``shuozi gateway`` and ``shuozi proxy`` subcommand parsers.
...
def build_gateway_parser(subparsers, *, cmd_gateway: Callable, cmd_proxy: Callable) -> None:
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if SHUOZI_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `shuozi proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
