"""
Security advisory checker for ShuoZi Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``shuozi doctor --ack <id>``; the ack is persisted to
...
1. ``shuozi doctor`` (and ``shuozi doctor --ack <id>``)
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including shuozi when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.shuozi/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: shuozi doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'shuozi doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.shuozi/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_shuozi_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``shuozi doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `shuozi doctor` on the gateway host for details.")
