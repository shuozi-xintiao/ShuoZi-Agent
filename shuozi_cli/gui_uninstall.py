...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``shuozi desktop`` / ``shuozi gui``)
     Built inside the agent checkout under ``$SHUOZI_HOME/shuozi-agent/``:
       ...
       - ``$SHUOZI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$SHUOZI_HOME``:
...
``shuozi uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(shuozi_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return shuozi_home / "shuozi-agent"
...
def source_built_gui_artifacts(shuozi_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``shuozi desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``shuozi-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(shuozi_home)
    ...
    return [
        ...
        shuozi_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(shuozi_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SHUOZI_HOME.
    ...
    if (agent_root / "shuozi_cli").is_dir():
        return True
...
def gui_is_installed(shuozi_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(shuozi_home):
        if p.exists():
            return True
...
def gui_install_summary(shuozi_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = shuozi_home if shuozi_home is not None else get_shuozi_home()
...
def uninstall_gui(shuozi_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``shuozi-agent/shuozi_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SHUOZI_HOME``.
