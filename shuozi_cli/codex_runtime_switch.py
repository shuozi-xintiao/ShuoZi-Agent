...
The actual runtime resolution happens in shuozi_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "shuozi"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        try:
            from shuozi_cli.codex_runtime_plugin_migration import migrate
            ...
            if "hermes-tools" in mig_report.migrated:
                ...
                msg_lines.append(
                    ...
                    "only on the default ShuoZi runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append("OpenAI/Codex turns will use the default ShuoZi runtime.")
