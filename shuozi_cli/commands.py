...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.shuozi/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update ShuoZi Agent to the latest version", "Info"),
    CommandDef("version", "Show ShuoZi Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`shuozi_cli.plugins.PluginContext.register_command`. They behave
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``shuozi skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``shuozi skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``shuozi skills list`` and
...
_SLACK_VIA_SHUOZI_ONLY = frozenset({"credits"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    entries.append(("shuozi", "Talk to Hermes or run a subcommand", "[subcommand] [args]"))
    seen.add("shuozi")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SHUOZI_ONLY:
            # Intentionally Slack-via-/hermes only (see _SLACK_VIA_SHUOZI_ONLY).
            return
...
def slack_app_manifest(request_url: str = "https://shuozi-agent.local/slack/commands") -> dict[str, Any]:
