...
the existing module-level functions in shuozi_cli.gateway and
shuozi_cli.gateway_windows directly. This protocol is a thin facade
...
``shuozi gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``shuozi gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    detection always fails. Since every ShuoZi runtime call inside the
    container drops to shuozi via ``s6-setuidgid``, that silent failure
...
# in ``shuozi_cli.gateway`` (systemd/launchd) and ``shuozi_cli.gateway_windows``
...
# active (set via the ``shuozi -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in shuozi_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from shuozi_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from shuozi_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from shuozi_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from shuozi_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in shuozi_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from shuozi_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from shuozi_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from shuozi_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from shuozi_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``shuozi_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from shuozi_cli import gateway_windows
        from shuozi_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `shuozi profile create`
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the shuozi user.
    ...
    is dispatched as the shuozi user.
    ...
    with shuozi ownership before triggering ``s6-svscanctl -a``,
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _SHUOZI_UID, _SHUOZI_GID)
        except PermissionError:
            # Running as the shuozi user already — directory is hermes-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _SHUOZI_UID, _SHUOZI_GID)
        except PermissionError:
            pass
    ...
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _SHUOZI_UID, _SHUOZI_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``shuozi -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`shuozi profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SHUOZI_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SHUOZI_HOME=/data/hermes`` is honored at run
          ...
          4. Drops to the shuozi user and exec's
             ``shuozi -p <profile> gateway run`` (or just ``hermes
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``shuozi gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SHUOZI_HOME profile" (the implicit profile that exists at
        the top of $SHUOZI_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``shuozi_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SHUOZI_HOME/profiles/default/`` — a
        ...
        ``shuozi_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        # guard.
        lines.append("export SHUOZI_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "shuozi gateway run"
        else:
            gateway_cmd = f"shuozi -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid shuozi {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SHUOZI_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SHUOZI_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SHUOZI_HOME=/data/hermes``
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``shuozi logs`` reads and what
        ...
        return (
            ...
            f': "${{SHUOZI_HOME:=/opt/data}}"\n'
            f'log_dir="$SHUOZI_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R hermes:hermes "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid shuozi s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``shuozi gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with shuozi ownership
            ...
            # the shuozi user) won't hit EACCES on root-owned 0700
