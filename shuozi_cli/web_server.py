"""
ShuoZi Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m shuozi_cli.main web          # Start on http://127.0.0.1:9119
    python -m shuozi_cli.main web --port 8080
"""
...
try:
    ...
    # running `shuozi dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["SHUOZI_WEB_DIST"]) if "SHUOZI_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``shuozi gateway run`` — but the
    desktop app spawns a ``shuozi dashboard`` backend, not a gateway, so a cron
    ...
    SHUOZI_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (SHUOZI_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("SHUOZI_DESKTOP") == "1":
...
app = FastAPI(title="ShuoZi Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# SHUOZI_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("SHUOZI_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``shuozi_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from shuozi_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from shuozi_cli.dashboard_auth.middleware import gated_auth_middleware
...
class ModelAssignment(BaseModel):
    ...
    # ``shuozi model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real ShuoZi provider slug — fine.
    ...
    1. Vendor-name → Hermes-provider mapping: when the provider string is
       not a known ShuoZi provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from shuozi_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from shuozi_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from shuozi_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "SHUOZI_DASHBOARD_FILES_ROOT"
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("SHUOZI_HOME", "").strip()
    if not raw:
        return False
    try:
...
async def get_status():
    ...
    # and which providers are registered so ``shuozi status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from shuozi_cli.dashboard_auth import list_providers as _list_providers
    ...
    return {
        ...
        "shuozi_home": str(get_shuozi_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the shuozi home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_shuozi_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from shuozi_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from shuozi_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from shuozi_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.shuozi/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_shuozi_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``shuozi <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``shuozi_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "shuozi_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SHUOZI_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``shuozi gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``shuozi gateway restart`` children race each other on the
...
async def restart_gateway():
    """Kick off a ``shuozi gateway restart`` in the background."""
    try:
...
async def update_hermes():
    """Kick off ``shuozi update`` in the background."""
    ...
    try:
        ...
        _log.exception("Failed to spawn shuozi update")
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``shuozi update``.
    ...
    try:
        ...
        if force:
            try:
                (get_shuozi_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.shuozi/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from shuozi_state import SessionDB
    from shuozi_cli import profiles as profiles_mod
...
# in shuozi_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    """
    try:
        from shuozi_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `shuozi model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `shuozi model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from shuozi_cli.auth import get_provider_auth_state
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.shuozi/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from shuozi_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (shuozi_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from shuozi_cli.nous_subscription import apply_nous_managed_defaults
                from shuozi_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``shuozi model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from shuozi_cli.main import _auto_provider_name, _save_custom_provider
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://shuozi-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "webhook": {
        ...
        "docs_url": "https://shuozi-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.shuozi-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "ShuoZi Agent").strip() or "ShuoZi Agent"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's SHUOZI_HOME.
...
# the canonical ``shuozi auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.shuozi/.anthropic_oauth.json`` — Hermes-managed PKCE flow (what
    ...
    try:
        from agent.anthropic_adapter import (
            read_shuozi_oauth_credentials,
            _SHUOZI_OAUTH_FILE,
        )
    except ImportError:
        read_shuozi_oauth_credentials = None  # type: ignore
        _SHUOZI_OAUTH_FILE = None  # type: ignore

    hermes_creds = None
    if read_shuozi_oauth_credentials:
        try:
            hermes_creds = read_shuozi_oauth_credentials()
        except Exception:
            hermes_creds = None
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "shuozi_pkce",
            "source_label": f"Hermes PKCE ({_SHUOZI_OAUTH_FILE})",
    ...
    try:
        ...
        pass
    try:
        from shuozi_cli.config import get_env_value
    except ImportError:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "shuozi auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shuozi auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "shuozi auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shuozi auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "shuozi auth add xai-oauth",
        "docs_url": "https://shuozi-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "shuozi auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from shuozi_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("shuozi_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _SHUOZI_OAUTH_FILE
            if _SHUOZI_OAUTH_FILE.exists():
                _SHUOZI_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from shuozi_cli.auth import clear_provider_auth
...
#          → persists to ~/.shuozi/.anthropic_oauth.json AND credential pool
...
# Guarded so shuozi web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``shuozi auth add anthropic``.
    """
    from agent.anthropic_adapter import _SHUOZI_OAUTH_FILE
    payload = {
    ...
    _SHUOZI_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _SHUOZI_OAUTH_FILE.with_name(
        f"{_SHUOZI_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _SHUOZI_OAUTH_FILE)
        try:
            _SHUOZI_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("SHUOZI_PORTAL_BASE_URL")
...
# the tokens exactly like `shuozi auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("SHUOZI_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `shuozi auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from shuozi_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from shuozi_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `shuozi sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `shuozi sessions prune`)."""
...
async def get_logs(
    ...
    from shuozi_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_shuozi_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from shuozi_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SHUOZI_HOME)."""
    from shuozi_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["shuozi_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SHUOZI_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (shuozi_cli.mcp_config), so
# servers managed here show up under `shuozi mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from shuozi_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from shuozi_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from shuozi_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from shuozi_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # SHUOZI_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds SHUOZI_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (shuozi_cli.webhook); the webhook
...
async def list_webhooks():
    import shuozi_cli.webhook as wh
...
# spawn the real `shuozi gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from shuozi_cli.auth import read_credential_pool
...
# `shuozi memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `shuozi memory setup` to configure a new one.",
            )
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `shuozi import`. The spawned action runs with
...
async def list_hooks():
    ...
    form can offer them.
    """
    from shuozi_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from shuozi_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <shuozi_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_shuozi_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``shuozi`` subprocess, and
...
# Human-readable labels for each hub source id (matches `shuozi skills search`
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from shuozi_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`shuozi -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the SHUOZI_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_shuozi_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "shuozi setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local SHUOZI_HOME override so the write lands in the target
    ...
    """
    from shuozi_constants import set_shuozi_home_override, reset_shuozi_home_override

    token = set_shuozi_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local SHUOZI_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the SHUOZI_HOME override. Returns the
    number of skills newly disabled.
    """
    from shuozi_constants import set_shuozi_home_override, reset_shuozi_home_override
    from shuozi_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's SHUOZI_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``shuozi profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from SHUOZI_HOME).
    """
    from shuozi_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``shuozi profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the SHUOZI_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``shuozi profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from shuozi_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_shuozi_home()`` at call
       time — the context-local override from ``set_shuozi_home_override``
    ...
    retargeted to the *current* ``get_shuozi_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a SHUOZI_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
async def get_skills(profile: Optional[str] = None):
    ...
    from shuozi_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from shuozi_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``shuozi tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``shuozi tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``shuozi tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.shuozi/.env`` via ``save_env_value`` —
    the same store ``shuozi tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``shuozi tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``shuozi -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SHUOZI_HOME the rest of the
...
#
# The endpoint spawns the same ``shuozi --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from shuozi_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from shuozi_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from shuozi_cli.dashboard_auth.audit import AuditEvent, audit_log
        from shuozi_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``shuozi --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SHUOZI_TUI_RESUME`` env var —
    matching what ``shuozi_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``SHUOZI_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``SHUOZI_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``SHUOZI_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_shuozi_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``shuozi -p <name>`` performs. The in-process
    ``SHUOZI_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from shuozi_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("SHUOZI_TUI_DISABLE_MOUSE", "1")
    env.setdefault("SHUOZI_TUI_INLINE", "1")

    if profile_dir is not None:
        env["SHUOZI_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["SHUOZI_TUI_RESUME"] = resume

    if sidecar_url:
        env["SHUOZI_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile SHUOZI_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["SHUOZI_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from shuozi_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
# SHUOZI_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`shuozi_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from shuozi_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__SHUOZI_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``__SHUOZI_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__SHUOZI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SHUOZI_BASE_PATH__="{prefix}";'
                f"window.__SHUOZI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__SHUOZI_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__SHUOZI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SHUOZI_BASE_PATH__="{prefix}";'
                f"window.__SHUOZI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.shuozi/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.shuozi/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_shuozi_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.shuozi/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as shuozi_cli.plugins):
    1. User plugins:    ~/.shuozi/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.shuozi/plugins/  (only if SHUOZI_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from shuozi_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_shuozi_home() / "plugins", "user"),
    ...
    # ``shuozi_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("SHUOZI_ENABLE_PROJECT_PLUGINS"):
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from shuozi_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_shuozi_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"shuozi auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from shuozi_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from shuozi_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shuozi_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shuozi_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.shuozi/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.shuozi/plugins/ if you trust it)",
...
from shuozi_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from shuozi_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing SHUOZI_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with ShuoZi Agent exposes a
    ...
    # SHUOZI_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"SHUOZI_DASHBOARD_READY port={actual_port}", flush=True)
