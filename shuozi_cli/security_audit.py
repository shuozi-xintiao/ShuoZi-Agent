"""On-demand supply-chain audit for ShuoZi Agent installs.
...
2. Python deps declared by user-installed plugins under ``~/.shuozi/plugins``
...
def _discover_plugins(shuozi_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.shuozi/plugins``.
...
def run_audit(
    *,
    ...
    home = shuozi_home or Path(get_shuozi_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, shuozi_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(shuozi_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `shuozi security audit`."""
    home = Path(get_shuozi_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, shuozi_home=home
    )
    if total == 0:
