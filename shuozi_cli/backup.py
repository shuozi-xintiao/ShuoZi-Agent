"""
Backup and import commands for shuozi CLI.

`shuozi backup` creates a zip archive of the entire ~/.shuozi/ directory
(excluding the shuozi-agent repo and transient files).

`shuozi import` restores from a backup zip, overlaying onto the current
SHUOZI_HOME root.
"""
...
from shuozi_constants import get_default_hermes_root, get_shuozi_home, display_shuozi_home
...
# ``shuozi-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/shuozi-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "shuozi-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to shuozi root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``shuozi-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/shuozi-agent/`` — must be preserved.
        if part == "shuozi-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SHUOZI_HOME.
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_shuozi_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``shuozi-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "shuozi-agent" and not is_root)
        ]
    ...
    print(f"\nRestore with: shuozi import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a shuozi home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.shuozi/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a shuozi dir name
        if prefix in {".hermes", "shuozi"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_shuozi_home()}")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # shuozi_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: shuozi profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "shuozi-agent").is_dir():
            print("Note: The shuozi-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: shuozi update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  shuozi -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and shuozi backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SHUOZI_HOME).
...
# — `shuozi update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(shuozi_home: Optional[Path] = None) -> Path:
    home = shuozi_home or get_shuozi_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    shuozi_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(shuozi_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SHUOZI_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``shuozi update``.
    ...
    Args:
        ...
        shuozi_home: Override for the Hermes home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(shuozi_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for shuozi backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_shuozi_home()}/state-snapshots/")
...
def _pre_update_backup_dir(shuozi_home: Optional[Path] = None) -> Path:
    home = shuozi_home or get_shuozi_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SHUOZI_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SHUOZI_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``shuozi update``) should continue even if the backup fails.
    """
    hermes_root = shuozi_home or get_default_hermes_root()
    if not hermes_root.is_dir():
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `shuozi claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SHUOZI_HOME under ``backups/`` before a
    ``shuozi claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``shuozi import <archive>``.  Writes to
    ``<SHUOZI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `shuozi import` and the
