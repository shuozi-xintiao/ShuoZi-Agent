# OpenAI-Compatible API Server for ShuoZi Agent
...
SSE streaming. By exposing this endpoint, shuozi-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  shuozi-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.shuozi/.env`
2. Run `shuozi gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with shuozi-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns shuozi-agent as a model) |
...
But shuozi-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "shuozi-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured shuozi-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of shuozi-agent's core prompt
...
Once implemented, shuozi-agent works as a drop-in backend for:
