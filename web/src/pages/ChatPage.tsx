/**
 * ChatPage — embeds `shuozi --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (shuozi_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { SHUOZI_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets SHUOZI_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${SHUOZI_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SHUOZI_SESSION_TOKEN__ &&
    !window.__SHUOZI_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `shuozi dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SHUOZI_AUTH_REQUIRED__;
    ...
    // WebSocket. In gated mode (``window.__SHUOZI_AUTH_REQUIRED__``) this
