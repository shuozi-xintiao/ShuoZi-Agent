...
// injects ``window.__SHUOZI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SHUOZI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SHUOZI_BASE_PATH = readBasePath();
const BASE = SHUOZI_BASE_PATH;
...
declare global {
  interface Window {
    __SHUOZI_SESSION_TOKEN__?: string;
    __SHUOZI_BASE_PATH__?: string;
    ...
    __SHUOZI_AUTH_REQUIRED__?: boolean;
  }
}
...
export async function fetchJSON<T>(
  ...
  const token = window.__SHUOZI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``shuozi update``, ``shuozi gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SHUOZI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SHUOZI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SHUOZI_SESSION_TOKEN__`` is valid, so the next
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__SHUOZI_SESSION_TOKEN__;
  if (injected) {
 ...
 *
 * The dashboard's gated-mode WS auth (``shuozi_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SHUOZI_AUTH_REQUIRED__) {
 ...
 * ``window.__SHUOZI_SESSION_TOKEN__`` directly, so the gated-mode ticket
