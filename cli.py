#!/usr/bin/env python3
"""
ShuoZi Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the ShuoZi Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills shuozi-agent-dev,github-auth
...
os.environ["SHUOZI_QUIET"] = "1"  # Our own modules

import yaml

from shuozi_cli.fallback_config import get_fallback_chain
from shuozi_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from shuozi_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from shuozi_cli.pt_input_extras import (
...
from shuozi_cli.banner import _format_context_length, format_banner_version_label
...
from shuozi_cli.browser_connect import (
...
load_hermes_dotenv(shuozi_home=_shuozi_home, project_env=_project_env)
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("SHUOZI_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.shuozi/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If SHUOZI_IGNORE_USER_CONFIG=1 is set (via ``shuozi chat --ignore-user-config``),
    the user config at ``~/.shuozi/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({SHUOZI_HOME}/config.yaml)
    user_config_path = _shuozi_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("SHUOZI_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see shuozi_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from shuozi_cli.config import _normalize_root_model_keys
    ...
    # Normalize config key: the new config system (shuozi_cli/config.py) and all
    ...
    # UNLESS we're inside a gateway process (detected by _SHUOZI_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_SHUOZI_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["SHUOZI_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.shuozi/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from shuozi_logging import setup_logging
...
try:
    from shuozi_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``shuozi --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from shuozi_cli.mcp_startup import wait_for_mcp_discovery
...
from shuozi_cli.banner import build_welcome_banner
from shuozi_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from shuozi_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("SHUOZI_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("SHUOZI_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run shuozi -w")
        return None
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`shuozi_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from shuozi_constants import get_shuozi_home as _get_shuozi_home
        _shuozi_home_maint = _get_shuozi_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_shuozi_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`shuozi_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    These are auto-generated by ``shuozi -w`` sessions and PR review
...
#
# Detection priority:
#   1. SHUOZI_LIGHT / SHUOZI_TUI_LIGHT env (true/false) — explicit override
#   2. SHUOZI_TUI_THEME=light|dark — explicit theme
#   3. SHUOZI_TUI_BACKGROUND=#RRGGBB — explicit bg hint
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("SHUOZI_LIGHT", "SHUOZI_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("SHUOZI_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("SHUOZI_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from shuozi_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from shuozi_cli.skin_engine import get_active_skin
...
from shuozi_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "ShuoZi Agent") if _skin else "ShuoZi Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("SHUOZI_FAST_STARTUP_BANNER") == "1":
        from shuozi_cli import __release_date__ as _release_date
        from shuozi_cli import __version__ as _version

        version_line = f"ShuoZi Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from shuozi_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.shuozi/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.shuozi/config.yaml on first use)
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the ShuoZi Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: SHUOZI_MAX_TOKENS)
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from shuozi_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("SHUOZI_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("SHUOZI_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("SHUOZI_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `shuozi chat --ignore-rules` in shuozi_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("SHUOZI_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("SHUOZI_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Hermes processes for this SHUOZI_HOME.
        ...
        self._history_file = _shuozi_home / ".hermes_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from shuozi_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from shuozi_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from shuozi_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from shuozi_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from shuozi_cli.codex_models import get_codex_model_ids
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``shuozi doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``shuozi doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        from shuozi_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with ShuoZi Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.shuozi/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from shuozi_cli.clipboard import save_clipboard_image

        img_dir = get_shuozi_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'shuozi setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from shuozi_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from shuozi_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from shuozi_cli.skin_engine import get_active_help_header
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from shuozi_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``shuozi sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `shuozi sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("SHUOZI_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.shuozi/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``shuozi --resume <id>``
        ...
        saved_dir = get_shuozi_home() / "sessions" / "saved"
        try:
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: shuozi --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from shuozi_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from shuozi_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `shuozi model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from shuozi_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from shuozi_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from shuozi_cli.model_switch import switch_model, parse_model_flags
        from shuozi_cli.providers import get_label
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from shuozi_cli import codex_runtime_switch as crs
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_shuozi_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_shuozi_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in shuozi_cli/commands.py instead of touching every dispatch site.
        from shuozi_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from shuozi_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from shuozi_cli.skin_engine import get_active_skin
            ...
            from shuozi_cli.main import _print_version_info
            ...
            from shuozi_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `shuozi plugins
                ...
                # catalog lives behind `shuozi plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: shuozi plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_shuozi_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: shuozi plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from shuozi_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: shuozi plugins list)")
                    print("  Enable/disable: shuozi plugins enable/disable <name>")
            except Exception as e:
            ...
            # Check for plugin-registered slash commands
                ...
                from shuozi_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from shuozi_cli.commands import COMMANDS
    ...
    def _toggle_verbose(self):
        ...
        from shuozi_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``SHUOZI_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``SHUOZI_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from shuozi_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by shuozi_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors SHUOZI_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `shuozi portal` to log in, then /credits.")
            return
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from shuozi_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.shuozi/skills/ and queue a note for the
        next user turn.
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _shuozi_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard SHUOZI_HOME, so no -p needed.
            try:
            ...
            print(f"  shuozi --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  shuozi -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from shuozi_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from shuozi_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from shuozi_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to ShuoZi Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to ShuoZi Agent! Type your message or /help for commands."
        ...
        try:
            from shuozi_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(SHUOZI_REDACT_SECRETS={_redact_raw}). "
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from shuozi_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in shuozi_cli.pt_input_extras
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from shuozi_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "ShuoZi Agent")
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``SHUOZI_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("SHUOZI_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the shuozi process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: shuozi setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `shuozi sessions list`
...
def _run_kanban_goal_loop_q(cli: "HermesCLI", first_response: str) -> None:
    ...
    only when ``SHUOZI_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("SHUOZI_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from shuozi_cli import kanban_db as _kb
    from shuozi_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    ShuoZi Agent CLI - Interactive AI Assistant
    ...
    os.environ["SHUOZI_INTERACTIVE"] = "1"
    ...
    # so main unwinds normally.  SHUOZI_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("SHUOZI_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``shuozi chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("SHUOZI_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from shuozi_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        if os.environ.get("SHUOZI_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("SHUOZI_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from shuozi_cli.kanban_db import (
                ...
                # Single-query mode (`shuozi chat -q "…"`): skip the welcome
