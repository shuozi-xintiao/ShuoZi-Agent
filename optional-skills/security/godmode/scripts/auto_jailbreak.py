...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHUOZI_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHUOZI_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
SHUOZI_HOME = Path(os.getenv("SHUOZI_HOME", Path.home() / ".hermes"))
CONFIG_PATH = SHUOZI_HOME / "config.yaml"
PREFILL_PATH = SHUOZI_HOME / "prefill.json"
