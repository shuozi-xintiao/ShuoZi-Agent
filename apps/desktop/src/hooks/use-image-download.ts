...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'shuozi:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.shuoziDesktop?.saveImageFromUrl) {
        if (await window.shuoziDesktop.saveImageFromUrl(src)) {
