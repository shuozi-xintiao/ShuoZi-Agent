...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { shuoziDesktop: { readDir: typeof readDir } }).shuoziDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { shuoziDesktop: unknown }).shuoziDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { shuoziDesktop: unknown }).shuoziDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.shuoziDesktop is missing', async () => {
    delete (window as unknown as { shuoziDesktop?: unknown }).shuoziDesktop
