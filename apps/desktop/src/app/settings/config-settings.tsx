...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<ShuoziConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<ShuoziConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getShuoziConfigRecord(), getShuoziConfigDefaults(), getShuoziConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: ShuoziConfigRecord) => {
