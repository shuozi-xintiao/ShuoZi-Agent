...
// backend now surfaces the full `shuozi model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in shuozi_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `shuozi model` curation). Best-effort — fall through to the refreshed
