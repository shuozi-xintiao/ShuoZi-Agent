...
interface ShuoziConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useShuoziConfig({ activeSessionIdRef, refreshProjectBranch }: ShuoziConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshShuoziConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getShuoziConfig(), getShuoziConfigDefaults().catch(() => ({}))])
  ...
  return { refreshShuoziConfig, sttEnabled, voiceMaxRecordingSeconds }
}
