...
const SKIN_KEY = 'shuozi-desktop-theme-v2'
const MODE_KEY = 'shuozi-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'shuozi-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'shuozi-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'shuozi-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.shuoziTheme = skinName
  root.dataset.shuoziMode = rendered
  ...
  window.shuoziDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('shuozi-boot-background', c.background)
    window.localStorage.setItem('shuozi-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.shuoziThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.shuoziDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active ShuoZi backend theme on connect. */
