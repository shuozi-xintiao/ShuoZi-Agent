...
export interface I18nConfigClient {
  getConfig: () => Promise<ShuoziConfigRecord>
  saveConfig: (config: ShuoziConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  ...
  saveConfig: config => {
    ...
    return saveShuoziConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ShuoziConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ShuoziConfigRecord, locale: Locale): ShuoziConfigRecord {
