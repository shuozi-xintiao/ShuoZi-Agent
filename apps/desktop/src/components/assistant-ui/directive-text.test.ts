...
import { formatRefValue, shuoziDirectiveFormatter } from './directive-text'
...
describe('shuoziDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = shuoziDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = shuoziDirectiveFormatter.parse('@file:src/main.tsx the entry point')
