...
const SHUOZI_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HermesRefType = (typeof SHUOZI_REF_TYPES)[number]
...
const SHUOZI_DIRECTIVE_RE = new RegExp(
...
export const shuoziDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SHUOZI_DIRECTIVE_RE)).map(match => {
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => shuoziDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.shuoziDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.shuoziDesktop?.readFileDataUrl(id)
