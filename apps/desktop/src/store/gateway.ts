...
const isOpen = (gateway: ShuoziGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ShuoziGateway | null>(null)
...
let primaryGateway: ShuoziGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ShuoziGateway | null, profile = 'default'): void {
...
export function activeGateway(): ShuoziGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new ShuoziGateway()
...
export async function ensureActiveGatewayOpen(): Promise<ShuoziGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
