 ...
 *     `X-ShuoZi-Session-Token` header; WS uses `?token=`.
...
// shuozi_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - shuozi_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - shuozi_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / shuozi #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-shuozi_session_at', '__Secure-shuozi_session_at', 'shuozi_session_at']
const RT_COOKIE_VARIANTS = ['__Host-shuozi_session_rt', '__Secure-shuozi_session_rt', 'shuozi_session_rt']
...
/**
 * True if any cookie in `cookies` is a shuozi session ACCESS-token cookie
