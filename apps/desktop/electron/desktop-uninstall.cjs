 ...
 * the three user-facing uninstall modes to the `shuozi uninstall` CLI flags,
 ...
 *              `shuozi uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `shuozi uninstall --yes`
...
/**
 * Map an uninstall mode to the `python -m shuozi_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `shuozi_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'shuozi_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/ShuoZi.app/Contents/MacOS/ShuoZi  → …/ShuoZi.app
 *   Windows: …\ShuoZi\ShuoZi.exe                 → …\ShuoZi  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/ShuoZi.app/Contents/MacOS/ShuoZi → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/ShuoZi.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs ShuoZi.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]ShuoZi$/i.test(dir) || /[\\/]shuozi-desktop$/i.test(dir)) return dir
    return null
  }
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import shuozi_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import shuozi_cli` resolves from source
...
function buildWindowsCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, shuoziHome }) {
  ...
  // a problem, but ShuoZi install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
