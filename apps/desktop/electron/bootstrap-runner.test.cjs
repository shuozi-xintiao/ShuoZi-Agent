...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'shuozi-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/shuozi-runner-test',
    sourceRepoRoot: null,
    shuoziHome: '/tmp/shuozi-runner-test',
    logRoot: '/tmp/shuozi-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'shuozi-agent', 'scripts')
