#!/usr/bin/env python3
"""
SQLite State Store for ShuoZi Agent.
...
DEFAULT_DB_PATH = get_shuozi_home() / "state.db"
...
# shuozi_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``shuozi_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see shuozi_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple shuozi processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``shuozi_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `shuozi update` to rebuild the venv with a "
    ...
    def delete_session_if_empty(
        ...
        and ``shuozi sessions list`` output. (Pattern ported from
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``SHUOZI_DISABLE_FTS_TRIGRAM`` or not yet created), so
