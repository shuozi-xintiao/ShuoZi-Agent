"""CLI entry point for the shuozi-agent ACP adapter.

Loads environment variables from ``~/.shuozi/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SHUOZI_HOME (default ``~/.shuozi``)."""
    from shuozi_cli.env_loader import load_hermes_dotenv

    shuozi_home = get_shuozi_home()
    loaded = load_hermes_dotenv(shuozi_home=shuozi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", shuozi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="shuozi-acp",
        description="Run ShuoZi Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print ShuoZi version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive ShuoZi provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.shuozi/node/ "
...
def _print_version() -> None:
    ...
    print(shuozi_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ShuoZiACPAgent  # noqa: F401

    print("ShuoZi ACP check OK")


def _run_setup() -> None:
    from shuozi_cli.main import main as shuozi_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "shuozi", "model"]
        shuozi_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``shuozi postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting shuozi-agent ACP adapter")
    ...
    import acp
    from .server import ShuoZiACPAgent
