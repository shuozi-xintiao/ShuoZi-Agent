...
Sessions are persisted to the shared SessionDB (``~/.shuozi/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    Windows ACP clients can launch ``shuozi acp`` inside WSL while still sending
    ...
    """
    from shuozi_constants import is_wsl
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["shuozi-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ShuoZi Agent."""
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current ShuoZi runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.shuozi/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SHUOZI_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_shuozi_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=shuozi_home / "state.db")
            return self._db_instance
        except Exception:
