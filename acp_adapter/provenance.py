...
This is an additive ShuoZi extension surfaced under ACP ``_meta.shuozi`` so
...
compression rotates the internal ShuoZi head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.shuozi.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_shuozi_session_id: The live internal ShuoZi DB session id
            (``state.agent.session_id``).
        previous_shuozi_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"shuozi": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_shuozi_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_shuozi_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_shuozi_session_id}
    ...
    rotated = bool(
        ...
        and previous_shuozi_session_id != current_shuozi_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentHermesSessionId": current_shuozi_session_id,
    ...
    if previous_shuozi_session_id:
        provenance["previousHermesSessionId"] = previous_shuozi_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"shuozi": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"shuozi": {"sessionProvenance": prov}}
