...
_SHUOZI_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on SHUOZI_DESKTOP
    ...
    # spawned as a kanban worker (SHUOZI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `shuozi tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SHUOZI_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in shuozi_cli/tools_config.py).
        "posture": True,
    },
    ...
    "hermes-cli": {
        ...
        "tools": _SHUOZI_CORE_TOOLS,
        "includes": []
    },

    "hermes-cron": {
        # Mirrors hermes-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `shuozi tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hermes-cli; gated by `shuozi tools`",
        "tools": _SHUOZI_CORE_TOOLS,
        "includes": []
    },
    ...
    "hermes-webhook": {
        ...
        "tools": _SHUOZI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "hermes-gateway": {
        ...
        "tools": [],
        "includes": ["hermes-telegram", "hermes-discord", "hermes-whatsapp", "hermes-slack", "hermes-signal", "hermes-bluebubbles", "shuozi-homeassistant", "hermes-email", "hermes-sms", "hermes-mattermost", "hermes-matrix", "hermes-dingtalk", "hermes-feishu", "hermes-wecom", "hermes-wecom-callback", "hermes-weixin", "hermes-qqbot", "hermes-webhook", "hermes-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        ...
        # Gives them _SHUOZI_CORE_TOOLS plus any tools the plugin registered
        ...
        if name.startswith("hermes-"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SHUOZI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
