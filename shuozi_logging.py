"""Centralized logging setup for ShuoZi Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.shuozi/logs/`` (profile-aware via ``get_shuozi_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``shuozi_cli.web_server``,
    ``shuozi_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from shuozi_constants import get_config_path, get_shuozi_home
...
# Used by _ComponentFilter and exposed for ``shuozi logs --component``.
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "shuozi_cli.web_server",
        "shuozi_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the shuozi group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from shuozi_cli.config import is_managed
