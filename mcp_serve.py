...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SHUOZI_HOME."""
    try:
        from shuozi_constants import get_shuozi_home
        return get_shuozi_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SHUOZI_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from shuozi_constants import get_shuozi_home
        directory_file = get_shuozi_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SHUOZI_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from shuozi_constants import get_shuozi_home
            db_file = get_shuozi_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SHUOZI_HOME", Path.home() / ".hermes")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "shuozi",
        instructions=(
            "ShuoZi Agent messaging bridge. Use these tools to interact with "
