...
# 5. Symlinks the 'shuozi' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ ShuoZi Agent Setup${NC}"
echo ""
...
SHUOZI_BIN="$SCRIPT_DIR/venv/bin/hermes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHUOZI_BIN" "$COMMAND_LINK_DIR/hermes"
...
# ============================================================================
# Seed bundled skills into ~/.shuozi/skills/
# ============================================================================

SHUOZI_SKILLS_DIR="${SHUOZI_HOME:-$HOME/.hermes}/skills"
mkdir -p "$SHUOZI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shuozi/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHUOZI_SKILLS_DIR/" 2>/dev/null || true
