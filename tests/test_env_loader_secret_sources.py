"""Tests for the secret-source tracking in ``shuozi_cli.env_loader``.

These cover the small public surface that lets `shuozi model` / `shuozi setup`
...
from shuozi_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, shuozi_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per SHUOZI_HOME per process.
    """
    ...
    # cli.py, shuozi_cli/main.py, run_agent.py, trajectory_compressor.py,
