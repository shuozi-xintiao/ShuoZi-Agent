"""Tests for shuozi_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hermes entry point
(hermes, shuozi-agent, shuozi-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports shuozi_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported shuozi_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("shuozi_bootstrap", None)
    import shuozi_bootstrap  # noqa: WPS433
    return shuozi_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import shuozi_bootstrap as its
    ...
    # import shuozi_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "shuozi_cli/main.py",   # shuozi CLI (console_script)
        "run_agent.py",          # shuozi-agent (console_script)
        "acp_adapter/entry.py",  # shuozi-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import shuozi_bootstrap' and that
        ...
        half-finished ``shuozi update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``shuozi_bootstrap`` as a top-level module) leaves hermes
        ...
        """
        # Resolve relative to the shuozi-agent repo root.  Tests live
        # at tests/test_shuozi_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # shuozi start even when shuozi_bootstrap hasn't been
        ...
        assert first_import_name == "shuozi_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'shuozi_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import shuozi_bootstrap' line to be the first import."
        )
