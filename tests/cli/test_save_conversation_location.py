...
file to be discoverable via ``shuozi sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.shuozi/sessions/saved/`` and prints
...
def shuozi_home(tmp_path, monkeypatch):
    ...
    # Clear any cached shuozi_home computation
    import shuozi_constants
    if hasattr(hermes_constants, "_shuozi_home_cache"):
        hermes_constants._shuozi_home_cache = None
    return home
...
def test_save_conversation_writes_under_shuozi_home(shuozi_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.shuozi/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the SHUOZI_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "shuozi_constants"]:
    ...
    # File MUST be under ~/.shuozi/sessions/saved/
    saved_dir = shuozi_home / "sessions" / "saved"
    ...
    assert "shuozi --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(shuozi_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "shuozi_constants"]:
        sys.modules.pop(mod, None)
    import cli
