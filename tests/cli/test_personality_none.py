...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._shuozi_home", tmp_path), \
             patch("hermes_constants.display_shuozi_home", return_value="~/.shuozi/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.shuozi/profiles/coder/config.yaml`"
