...
engine plugins (e.g. shuozi-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``shuozi plugins`` picker."""
    from shuozi_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "shuozi_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "shuozi_cli.plugins.get_plugin_context_engine",
