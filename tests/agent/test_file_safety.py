...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        """Hermes' own .env inside SHUOZI_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: shuozi internal .env is in ~/.shuozi/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.shuozi/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.shuozi/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = shuozi_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._shuozi_home_path", return_value=shuozi_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal ShuoZi cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        shuozi_home = tmp_path / ".hermes"
        hub = shuozi_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_shuozi_home(self, tmp_path):
        """The env basename guard does not depend on SHUOZI_HOME resolution."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        shuozi_home = tmp_path / ".hermes"
        cache = shuozi_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
