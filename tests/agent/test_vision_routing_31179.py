...
def isolated_home(monkeypatch):
    """Temp SHUOZI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hermes_test_31179_")
    shuozi_home = os.path.join(test_home, ".hermes")
    os.makedirs(shuozi_home)
...
def _fresh_modules():
    """Drop cached shuozi modules so each test reloads against current env."""
