...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $SHUOZI_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $SHUOZI_HOME/plugins/."""
        from plugins.memory import load_memory_provider
        self._make_user_memory_plugin(tmp_path, "myexternal")
        monkeypatch.setattr(
            "plugins.memory._get_user_plugins_dir",
    ...
    def test_load_user_plugin_with_nested_subpackage(self, tmp_path, monkeypatch):
        ...
        impl_dir = plugin_dir / "adapters" / "shuozi"  # adapters/ has no __init__.py
...
class TestSetupFieldFiltering:
    ...
    memory setup wizard in shuozi_cli/memory_setup.py.
