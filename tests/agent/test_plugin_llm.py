...
""",
        ...
        from shuozi_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        from shuozi_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from shuozi_cli.plugins import PluginContext, PluginManifest, PluginManager
