...
def test_main_wrapper_preserves_docker_workdir() -> None:
    ...
    assert 'SHUOZI_ORIG_CWD:-$PWD' in text, (
...
def test_dashboard_run_resets_home_before_dropping_privileges() -> None:
    ...
    assert "exec s6-setuidgid shuozi hermes dashboard" in text
...
def test_dashboard_run_does_not_derive_insecure_from_bind_host() -> None:
    ...
    ``SHUOZI_DASHBOARD_HOST``. Doing so disables the OAuth auth gate on
    ...
    The opt-in is now explicit: ``SHUOZI_DASHBOARD_INSECURE=1`` (truthy).
    ...
    assert '127.0.0.1|localhost' not in text, (
        ...
        "is the authority now — opt in via SHUOZI_DASHBOARD_INSECURE instead."
    )
    ...
    assert "SHUOZI_DASHBOARD_INSECURE" in text, (
        "Explicit SHUOZI_DASHBOARD_INSECURE opt-in is missing."
    )
    # Truthy values aligned with the rest of the s6 scripts
    # (e.g. SHUOZI_DASHBOARD).
    for truthy in ("1", "true", "TRUE", "True", "yes", "YES", "Yes"):
        assert truthy in text, (
            f"SHUOZI_DASHBOARD_INSECURE should accept truthy value {truthy!r}"
        )
...
def test_stage2_hook_repairs_profiles_and_cron_ownership_on_every_boot() -> None:
    ...
    assert 'if [ -d "$SHUOZI_HOME/profiles" ]; then' in text
    assert 'chown -R hermes:hermes "$SHUOZI_HOME/profiles" 2>/dev/null || true' in text

    assert 'if [ -d "$SHUOZI_HOME/cron" ]; then' in text
    assert 'chown -R hermes:hermes "$SHUOZI_HOME/cron" 2>/dev/null || true' in text
