...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        shuozi_home.mkdir()
        (shuozi_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"SHUOZI_HOME": str(shuozi_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(shuozi_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.shuozi/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.shuozi/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        shuozi_home = tmp_path / ".hermes"
        (shuozi_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SHUOZI_HOME": str(shuozi_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.hermes")

        assert mounts[0]["container_path"] == "/home/user/.shuozi/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.shuozi/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.shuozi/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SHUOZI_HOME."""
        monkeypatch.setenv("SHUOZI_HOME", str(tmp_path / ".hermes"))
        (tmp_path / ".hermes").mkdir()

        # Create a sensitive file one level above shuozi_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside shuozi_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SHUOZI_HOME must still be registered."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        (shuozi_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_shuozi_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SHUOZI_HOME must be allowed."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        subdir = shuozi_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SHUOZI_HOME pointing outside must be rejected."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

        # Create a sensitive file outside shuozi_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside shuozi_home pointing outside
        symlink = shuozi_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SHUOZI_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, shuozi_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SHUOZI_HOME."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        ...
        sensitive.write_text("{}")
        self._write_config(shuozi_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(shuozi_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SHUOZI_HOME via config must still mount."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

        (shuozi_home / "oauth.json").write_text("{}")
        self._write_config(shuozi_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "cache" / "documents").mkdir(parents=True)
        (shuozi_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.shuozi/cache/documents" in paths
        assert "/root/.shuozi/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.shuozi/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        (shuozi_home / "document_cache").mkdir()
        (shuozi_home / "image_cache").mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(shuozi_home / "document_cache") in host_paths
        assert str(shuozi_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.shuozi/cache/documents" in container_paths
        assert "/root/.shuozi/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        shuozi_home = tmp_path / ".hermes"
        img_dir = shuozi_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.shuozi/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/home/agent/.shuozi/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        shuozi_home = tmp_path / ".hermes"
        (shuozi_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(shuozi_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        doc_dir = shuozi_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        ss_dir = shuozi_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.shuozi/cache/screenshots/session_abc/screen1.png"
