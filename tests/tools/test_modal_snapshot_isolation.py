...
def _restore_tool_modules():
    original_shuozi_home = os.environ.get("SHUOZI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_shuozi_home is None:
            os.environ.pop("SHUOZI_HOME", None)
        else:
            os.environ["SHUOZI_HOME"] = original_shuozi_home
        _reset_modules(("tools", "shuozi_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    shuozi_cli = types.ModuleType("shuozi_cli")
    shuozi_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["shuozi_cli"] = shuozi_cli
    shuozi_home = tmp_path / "shuozi-home"
    os.environ["SHUOZI_HOME"] = str(shuozi_home)
    sys.modules["shuozi_cli.config"] = types.SimpleNamespace(
        get_shuozi_home=lambda: shuozi_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="shuozi-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": shuozi_home / "modal_snapshots.json",
