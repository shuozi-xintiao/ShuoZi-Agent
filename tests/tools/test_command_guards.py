...
def _clean_state():
    ...
    saved = {}
    for k in ("SHUOZI_INTERACTIVE", "SHUOZI_GATEWAY_SESSION", "SHUOZI_EXEC_ASK", "SHUOZI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SHUOZI_INTERACTIVE"] = "1"
        session_key = os.getenv("SHUOZI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SHUOZI_INTERACTIVE or SHUOZI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["SHUOZI_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("SHUOZI_GATEWAY_SESSION", None)
            os.environ.pop("SHUOZI_EXEC_ASK", None)
            os.environ.pop("SHUOZI_SESSION_KEY", None)
