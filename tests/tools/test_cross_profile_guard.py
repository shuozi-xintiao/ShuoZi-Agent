...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Hermes layout and point SHUOZI_HOME at
    ...
    monkeypatch.setenv("SHUOZI_HOME", str(sec_home))

    import shuozi_constants
    monkeypatch.setattr(hermes_constants, "get_default_hermes_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_shuozi_home_path", lambda: sec_home)
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up SHUOZI_HOME (set in
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.shuozi/profiles/<name>/."""
        # Don't set SHUOZI_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_shuozi_home_path", lambda: tmp_path / "fake")
