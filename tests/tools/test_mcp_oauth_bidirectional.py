...
async def test_hermes_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _SHUOZI_PROVIDER_CLS, reset_manager_for_tests

    assert _SHUOZI_PROVIDER_CLS is not None, "SDK OAuth types must be available"
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="ShuoZi Agent",
    )
    provider = _SHUOZI_PROVIDER_CLS(
        server_name="srv",
