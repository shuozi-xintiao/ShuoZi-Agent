...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active SHUOZI_HOME (profile-aware, not just
        # ``~/.shuozi``) must be write-denied. The hermetic test conftest
        # points SHUOZI_HOME at a tempdir — resolve via get_shuozi_home()
        # to match the denylist.
        from shuozi_constants import get_shuozi_home
        path = str(get_shuozi_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.shuozi/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: SHUOZI_HOME does point to the profile dir, not the root.
        from shuozi_constants import get_shuozi_home, get_default_hermes_root
        assert get_shuozi_home() == profile_home
