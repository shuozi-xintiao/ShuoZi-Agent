...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``SHUOZI_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # SHUOZI_* handling (#27303): only the operational allowlist passes;
        # every other SHUOZI_* is dropped (the broad prefix was removed).
        "SHUOZI_HOME": "/home/alice/.hermes",        # allowlisted → kept
        "SHUOZI_PROFILE": "default",                 # allowlisted → kept
        "SHUOZI_INTERACTIVE": "1",                   # not allowlisted → dropped
        "SHUOZI_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "SHUOZI_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
