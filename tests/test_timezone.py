"""
Tests for timezone support (shuozi_time module + integration points).
...
def _reset_shuozi_time_cache():
    """Reset the shuozi_time module cache (replacement for removed reset_cache)."""
    shuozi_time._cached_tz = None
    shuozi_time._cached_tz_name = None
    shuozi_time._cache_resolved = False


# =========================================================================
# shuozi_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def setup_method(self):
        _reset_shuozi_time_cache()

    def teardown_method(self):
        _reset_shuozi_time_cache()
        os.environ.pop("SHUOZI_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["SHUOZI_TIMEZONE"] = "Asia/Kolkata"
        result = shuozi_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["SHUOZI_TIMEZONE"] = "UTC"
        result = shuozi_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["SHUOZI_TIMEZONE"] = "America/New_York"
        result = shuozi_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["SHUOZI_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="shuozi_time"):
            result = shuozi_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SHUOZI_TIMEZONE"] = "Europe/London"
        tz = shuozi_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["SHUOZI_TIMEZONE"] = "Not/A/Timezone"
        tz = shuozi_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SHUOZI_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SHUOZI_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SHUOZI_TIMEZONE=" + os.environ.get("SHUOZI_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SHUOZI_TIMEZONE=NOT_SET" in result["output"], (
            "SHUOZI_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SHUOZI_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SHUOZI_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _shuozi_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SHUOZI_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_shuozi_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SHUOZI_TIMEZONE"] = "US/Eastern"
        _reset_shuozi_time_cache()
