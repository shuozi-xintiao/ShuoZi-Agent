...
normal tests assume away:
  ...
  - Filesystem: SHUOZI_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own SHUOZI_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("shuozi_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from shuozi_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "SHUOZI_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from shuozi_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["SHUOZI_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE shuozi_home: {resolved}")
...
@scenario("shuozi_home_with_spaces")
def _(home, kb):
    """SHUOZI_HOME at a path with spaces — should work but catches
    ...
    weird = tempfile.mkdtemp(prefix="shuozi with spaces ")
    os.environ["SHUOZI_HOME"] = weird
    ...
    try:
        ...
        print(f"  SHUOZI_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("shuozi_home_with_unicode")
def _(home, kb):
    """SHUOZI_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  SHUOZI_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("shuozi_home_via_symlink")
def _(home, kb):
    """SHUOZI_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same SHUOZI_HOME share DB correctly")
    finally:
...
def _idempotency_race_worker(shuozi_home: str, key: str, result_file: str,
...
def _(home, kb):
    ...
    try:
        from shuozi_cli import web_server as ws  # noqa
    except Exception:
        pass
