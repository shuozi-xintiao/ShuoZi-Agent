"""Shared fixtures for the shuozi-agent test suite.
...
2. **Isolated SHUOZI_HOME.** SHUOZI_HOME points to a per-test tempdir so
   code reading ``~/.shuozi/*`` via ``get_shuozi_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_shuozi_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SHUOZI_SESSION_* inheritance** — the agent's current gateway
...
# SHUOZI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SHUOZI_BEHAVIORAL_VARS = frozenset({
    "SHUOZI_YOLO_MODE",
    "SHUOZI_INTERACTIVE",
    "SHUOZI_QUIET",
    "SHUOZI_TOOL_PROGRESS",
    "SHUOZI_TOOL_PROGRESS_MODE",
    "SHUOZI_MAX_ITERATIONS",
    "SHUOZI_SESSION_PLATFORM",
    "SHUOZI_SESSION_CHAT_ID",
    "SHUOZI_SESSION_CHAT_NAME",
    "SHUOZI_SESSION_THREAD_ID",
    "SHUOZI_SESSION_SOURCE",
    "SHUOZI_SESSION_KEY",
    "SHUOZI_GATEWAY_SESSION",
    "SHUOZI_CRON_SESSION",
    "_SHUOZI_GATEWAY",
    "SHUOZI_PLATFORM",
    "SHUOZI_MODEL",
    "SHUOZI_INFERENCE_MODEL",
    "SHUOZI_INFERENCE_PROVIDER",
    "SHUOZI_TUI_PROVIDER",
    "SHUOZI_MANAGED",
    "SHUOZI_DEV",
    "SHUOZI_CONTAINER",
    ...
    "SHUOZI_TIMEZONE",
    "SHUOZI_REDACT_SECRETS",
    "SHUOZI_BACKGROUND_NOTIFICATIONS",
    "SHUOZI_EXEC_ASK",
    "SHUOZI_HOME_MODE",
    "SHUOZI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.shuozi/kanban.db instead of the per-test SHUOZI_HOME.
    "SHUOZI_KANBAN_DB",
    "SHUOZI_KANBAN_BOARD",
    "SHUOZI_KANBAN_HOME",
    "SHUOZI_KANBAN_WORKSPACES_ROOT",
    "SHUOZI_KANBAN_LOGS_ROOT",
    "SHUOZI_KANBAN_TASK",
    "SHUOZI_KANBAN_WORKSPACE",
    "SHUOZI_KANBAN_RUN_ID",
    "SHUOZI_KANBAN_CLAIM_LOCK",
    "SHUOZI_KANBAN_DISPATCH_IN_GATEWAY",
    "SHUOZI_TENANT",
    ...
    "SHUOZI_DASHBOARD_OAUTH_CLIENT_ID",
    "SHUOZI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SHUOZI_HOME to per-test tempdirs so code that
    reads ``~/.shuozi/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SHUOZI_* vars that could change test semantics.
    for name in _SHUOZI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect SHUOZI_HOME to a per-test tempdir. Code that reads
    #    ``~/.shuozi/*`` via ``get_shuozi_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.shuozi/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_shuozi_home()``
    #    is a bug to fix at the callsite.
    fake_shuozi_home = tmp_path / "hermes_test"
    fake_shuozi_home.mkdir()
    (fake_shuozi_home / "sessions").mkdir()
    (fake_shuozi_home / "cron").mkdir()
    (fake_shuozi_home / "memories").mkdir()
    (fake_shuozi_home / "skills").mkdir()
    monkeypatch.setenv("SHUOZI_HOME", str(fake_shuozi_home))
    ...
    #    ~/.shuozi/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import shuozi_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_shuozi_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal shuozi config dict suitable for unit tests."""
    return {
...
# correlated with a ``tests/shuozi_cli/`` pytest run starting up.
#
...
def _live_system_guard(request, monkeypatch):
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SHUOZI_TOKENS)
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m shuozi_cli.main".
                if (
                    "shuozi" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `shuozi update` (or the
        # equivalent `python -m shuozi_cli.main update`).  These commands
        ...
        if "update" in low and (
            # shuozi update / shuozi update --gateway / setsid bash -c ... shuozi update
            ("shuozi" in low and "update" in low.split())
            or
            # python -m shuozi_cli.main update --gateway
            ("shuozi_cli" in low and "update" in low.split())
            or
            ...
            raise RuntimeError(
                ...
                "`shuozi update` against the real checkout, fetching "
