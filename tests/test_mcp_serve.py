...
def _isolate_shuozi_home(tmp_path, monkeypatch):
    """Redirect SHUOZI_HOME to a temp directory."""
    monkeypatch.setenv("SHUOZI_HOME", str(tmp_path))
    try:
        import shuozi_constants
        monkeypatch.setattr(hermes_constants, "get_shuozi_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking shuozi_state schema."""
