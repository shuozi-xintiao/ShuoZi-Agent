...
class TestGetDefaultHermesRoot:
    ...
    def test_no_shuozi_home_returns_native(self, tmp_path, monkeypatch):
        """When SHUOZI_HOME is not set, returns ~/.shuozi."""
        monkeypatch.delenv("SHUOZI_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".hermes"

    def test_shuozi_home_is_native(self, tmp_path, monkeypatch):
        """When SHUOZI_HOME = ~/.shuozi, returns ~/.shuozi."""
        native = tmp_path / ".hermes"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_shuozi_home_is_profile(self, tmp_path, monkeypatch):
        """When SHUOZI_HOME is a profile under ~/.shuozi, returns ~/.shuozi."""
        native = tmp_path / ".hermes"
    ...
    def test_shuozi_home_is_docker(self, tmp_path, monkeypatch):
        """When SHUOZI_HOME points outside ~/.shuozi (Docker), returns SHUOZI_HOME."""
    ...
    def test_shuozi_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SHUOZI_HOME outside ~/.shuozi is treated as the root."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SHUOZI_HOME=<root>/profiles/<name>),
    ...
    def test_no_shuozi_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\hermes, not ~/.shuozi."""
        ...
        assert get_default_hermes_root() == local_appdata / "shuozi"

    def test_no_shuozi_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        ...
        assert get_default_hermes_root() == home / "AppData" / "Local" / "shuozi"


class TestGetHermesHome:
    """Tests for get_shuozi_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When SHUOZI_HOME is unset on Windows, use %LOCALAPPDATA%\\hermes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SHUOZI_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_shuozi_home() == local_appdata / "shuozi"
