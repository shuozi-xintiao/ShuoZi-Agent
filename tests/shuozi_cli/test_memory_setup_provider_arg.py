"""Tests for `shuozi memory setup [provider]` routing.
...
``shuozi memory setup honcho``) without the interactive picker — which matters
because the per-provider ``shuozi <provider>`` subcommand is only registered
...
from shuozi_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "shuozi memory setup" in out
