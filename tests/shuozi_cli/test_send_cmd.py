"""Tests for the ``shuozi send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``shuozi_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from shuozi_cli import send_cmd
...
def test_load_hermes_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``shuozi send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``shuozi config set``) lives
    ...
    shuozi_home.mkdir()
    (shuozi_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_shuozi_home() to re-resolve under the patched env.
    from importlib import reload

    import shuozi_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_hermes_env_does_not_override_existing(tmp_path, monkeypatch):
    ...
    shuozi_home = tmp_path / ".hermes"
    shuozi_home.mkdir()
    (shuozi_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
