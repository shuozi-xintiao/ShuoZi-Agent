"""Tests for `shuozi fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SHUOZI_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from shuozi_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from shuozi_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("shuozi_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("shuozi_cli.main._require_tty"):
            from shuozi_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from shuozi_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("shuozi_cli.setup._curses_prompt_choice", return_value=1):
            from shuozi_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from shuozi_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `shuozi fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shuozi_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
