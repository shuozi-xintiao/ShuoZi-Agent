"""Tests for Codex auth — tokens stored in Hermes auth store (~/.shuozi/auth.json)."""
...
def _setup_hermes_auth(shuozi_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(shuozi_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(shuozi_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
    ...
    monkeypatch.setattr("shuozi_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    shuozi_home = tmp_path / "shuozi"
    _setup_hermes_auth(shuozi_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((shuozi_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``shuozi auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``shuozi auth add openai-codex``.
    ...
    (shuozi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `shuozi auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``shuozi auth add openai-codex`` for accounts B and C
    ...
    (shuozi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``shuozi auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``shuozi auth add openai-codex`` as a workaround when there was no
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run shuozi auth".
    """
    from shuozi_cli.auth import (
    ...
    assert "shuozi auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from shuozi_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from shuozi_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "shuozi_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("shuozi_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("shuozi_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
