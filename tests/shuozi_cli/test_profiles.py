"""Comprehensive tests for shuozi_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SHUOZI_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.shuozi/profiles)
    * SHUOZI_HOME  -> tmp_path/.hermes  (so get_shuozi_home() agrees)
    * Creates the bare-minimum ~/.shuozi directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["shuozi", "test", "tmp", "root", "sudo"])
...
class TestGetProfileDir:
    ...
    def test_default_returns_shuozi_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.shuozi must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes shuozi-agent, .worktrees,
        ...
        (default_home / "shuozi-agent" / ".git").mkdir(parents=True)
        (default_home / "shuozi-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "shuozi-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "shuozi-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `shuozi profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `shuozi update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `shuozi update` pass that
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("shuozi_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("shuozi_cli.profiles._cleanup_gateway_service"), \
             patch("shuozi_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_shuozi_home_returns_default(self, profile_env):
        # SHUOZI_HOME points to tmp_path/.hermes which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom SHUOZI_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("SHUOZI_HOME", str(custom))
        # With Docker-aware roots, a custom SHUOZI_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("shuozi")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from shuozi_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "shuozi -p mybot" in content
    ...
    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from shuozi_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from shuozi_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "shuozi -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from shuozi_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from shuozi_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from shuozi_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("shuozi_cli.profiles.check_alias_collision", return_value="skip"):
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("shuozi-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/shuozi-agent", "default/.worktrees", "default/profiles",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_shuozi_home()."""
    ...
    def test_default_shuozi_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_shuozi_home()
        assert home == tmp_path / ".hermes"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (SHUOZI_HOME outside ~/.shuozi), profiles go under SHUOZI_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_shuozi_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_shuozi_home() returns SHUOZI_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (SHUOZI_HOME under ~/.shuozi), profiles root is still ~/.shuozi/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under SHUOZI_HOME."""
        from shuozi_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under SHUOZI_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from shuozi_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
