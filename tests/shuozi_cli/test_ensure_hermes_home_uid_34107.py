"""Regression tests for #34107 — Docker UID/GID handling in ensure_shuozi_home.

When Hermes runs in Docker with ``SHUOZI_UID=1000`` / ``SHUOZI_GID=911``,
the entrypoint chowns the top-level ``SHUOZI_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SHUOZI_UID", "1000")
        monkeypatch.setenv("SHUOZI_GID", "911")
        from shuozi_cli.config import _resolve_hermes_uid_gid
        uid, gid = _resolve_hermes_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SHUOZI_UID", raising=False)
        monkeypatch.delenv("SHUOZI_GID", raising=False)
        from shuozi_cli.config import _resolve_hermes_uid_gid
        uid, gid = _resolve_hermes_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("SHUOZI_UID", "not-a-number")
        monkeypatch.setenv("SHUOZI_GID", "911")
        from shuozi_cli.config import _resolve_hermes_uid_gid
        uid, gid = _resolve_hermes_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("SHUOZI_UID", "")
        monkeypatch.setenv("SHUOZI_GID", "")
        from shuozi_cli.config import _resolve_hermes_uid_gid
        uid, gid = _resolve_hermes_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SHUOZI_UID", "1000")
        monkeypatch.setenv("SHUOZI_GID", "911")
        from shuozi_cli import config as cfg
