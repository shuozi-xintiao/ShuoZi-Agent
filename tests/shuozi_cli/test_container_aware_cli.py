...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SHUOZI_HOME with .container-mode file."""
    shuozi_home = tmp_path / ".hermes"
    shuozi_home.mkdir()
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
    monkeypatch.delenv("SHUOZI_DEV", raising=False)

    container_mode = shuozi_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=shuozi-agent\n"
        "exec_user=hermes\n"
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["container_name"] == "shuozi-agent"
    assert info["exec_user"] == "shuozi"
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when SHUOZI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SHUOZI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """SHUOZI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SHUOZI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        shuozi_home = Path(tmpdir) / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("hermes_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_shuozi_home": lambda: shuozi_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHUOZI_DEV", None)
            info = get_container_exec_info()
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from shuozi_cli.main import _exec_in_container
