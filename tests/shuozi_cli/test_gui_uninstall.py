"""Tests for shuozi_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import shuozi_cli.gui_uninstall as gu


def _make_agent(shuozi_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = shuozi_home / "shuozi-agent"
    (agent_root / "shuozi_cli").mkdir(parents=True)
    (agent_root / "shuozi_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(shuozi_home: Path) -> None:
    """Create the source-built GUI artifacts a `shuozi desktop` run produces."""
    desktop = shuozi_home / "shuozi-agent" / "apps" / "desktop"
    ...
    (shuozi_home / "shuozi-agent" / "node_modules").mkdir(parents=True)
    (shuozi_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(shuozi_home: Path) -> None:
    (shuozi_home / "config.yaml").write_text("x: 1\n")
    (shuozi_home / ".env").write_text("KEY=secret\n")
    (shuozi_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    shuozi_home = tmp_path / ".hermes"
    shuozi_home.mkdir()
    assert gu.agent_is_installed(shuozi_home) is False
    _make_agent(shuozi_home)
    assert gu.agent_is_installed(shuozi_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    shuozi_home = tmp_path / ".hermes"
    (shuozi_home / "shuozi-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(shuozi_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    shuozi_home = tmp_path / ".hermes"
    _make_gui_build(shuozi_home)
    artifacts = gu.source_built_gui_artifacts(shuozi_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(shuozi_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(shuozi_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    shuozi_home = tmp_path / ".hermes"
    agent_root = _make_agent(shuozi_home)
    _make_gui_build(shuozi_home)
    _make_user_data(shuozi_home)
    ...
    removed = gu.uninstall_gui(shuozi_home)
    ...
    assert not (shuozi_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "shuozi_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (shuozi_home / "config.yaml").exists()
    assert (shuozi_home / ".env").exists()
    assert (shuozi_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(shuozi_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(shuozi_home)
    ...
    assert summary["shuozi_home"] == str(shuozi_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    SHUOZI_HOME with all the destructive externals stubbed out.
    """
    import shuozi_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_shuozi_home", lambda: shuozi_home)
    ...
    from shuozi_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (shuozi_home / "config.yaml").exists()
    assert shuozi_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole SHUOZI_HOME non-interactively."""
    import shuozi_cli.uninstall as uninstall

    shuozi_home = tmp_path / ".hermes"
    (shuozi_home / "shuozi-agent" / "shuozi_cli").mkdir(parents=True)
    (shuozi_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not shuozi_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m shuozi_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_shuozi_home", lambda: shuozi_home)
    ...
    assert not (desktop / "release").exists()
    assert not (shuozi_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "shuozi_cli").exists()
    assert (shuozi_home / "config.yaml").exists()
