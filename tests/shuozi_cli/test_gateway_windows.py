"""Tests for shuozi_cli.gateway_windows."""

from pathlib import Path

import pytest

import shuozi_cli.gateway as gateway
import shuozi_cli.gateway_windows as gateway_windows
import shuozi_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    shuozi_home = tmp_path / "shuozi-home"
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda shuozi_home: "")
    monkeypatch.setattr("shuozi_cli.config.get_shuozi_home", lambda: str(shuozi_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "shuozi_cli.main"]
    assert cwd == str(shuozi_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_shuozi_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr("shuozi_cli.config.get_shuozi_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("shuozi_cli.config.get_shuozi_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_shuozi_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    shuozi_home = tmp_path / "shuozi-home"
    shuozi_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(shuozi_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Hermes\\shuozi-agent\\venv\\Scripts\\python.exe",
        r"C:\\Hermes\\shuozi-agent",
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "shuozi --profile alice gateway start" in out
...
# resume_pending=True) never fires when `shuozi gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("shuozi_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("shuozi_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `shuozi gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
    ...
    monkeypatch.setattr("shuozi_cli.gateway._get_restart_drain_timeout", lambda: 1.0)
