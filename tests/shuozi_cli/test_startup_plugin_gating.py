...
``shuozi_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``shuozi --help``, ``shuozi version``,
``shuozi logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``shuozi --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from shuozi_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["shuozi", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["shuozi", "-w", "--tui"], None),
        (["shuozi", "version"], "version"),
        (["shuozi", "--tui", "chat"], "chat"),
        (["shuozi", "-w", "logs"], "logs"),
        (["shuozi", "chat", "hello world"], "chat"),
        (["shuozi", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["shuozi", "-m", "gpt5", "chat"], "chat"),
        (["shuozi", "--model", "gpt5", "chat", "hi"], "chat"),
        (["shuozi", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["shuozi", "-z", "hello world"], None),
        (["shuozi", "-z", "hello", "chat"], "chat"),
        (["shuozi", "--model=gpt5", "chat"], "chat"),     # inline form
        (["shuozi", "--", "chat"], "chat"),               # -- terminator
        (["shuozi", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["shuozi", "some-plugin-cmd"], "some-plugin-cmd"),
        (["shuozi", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["shuozi"],                          # bare → chat
        ["shuozi", "--help"],                # top-level help
        ["shuozi", "-h"],
        ["shuozi", "version"],               # known built-in
        ["shuozi", "logs"],
        ["shuozi", "gateway", "run"],
        ["shuozi", "--tui"],
        ["shuozi", "-w", "--tui"],
        ["shuozi", "chat", "hi"],
        ["shuozi", "help"],                  # accepted built-in-ish
        ["shuozi", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["shuozi", "meet", "join"],          # potential google_meet plugin
        ["shuozi", "honcho", "status"],      # potential memory plugin
        ["shuozi", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``shuozi help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"shuozi_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
