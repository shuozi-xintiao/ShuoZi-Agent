"""Tests for shuozi_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "shuozi_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="ShuoZi Agent")
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "shuozi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("shuozi_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("shuozi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("shuozi_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("shuozi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("shuozi_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "shuozi_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("shuozi_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("shuozi_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from shuozi_cli.setup import _setup_telegram_auto
