...
def _client():
    ...
    # Keep the state DB under the isolated SHUOZI_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_shuozi_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_shuozi_home):
        ...
        (get_shuozi_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "shuozi_version"):
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "shuozi_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/hermes/update`` runs ``shuozi update``.
    """
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import shuozi_cli.debug as dbg
        ...
        monkeypatch.setattr("shuozi_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `shuozi tools` configurator."""
