...
import shuozi_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "shuozi gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`shuozi gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `shuozi gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `shuozi gateway restart`)."""
        unit_path = tmp_path / "hermes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``SHUOZI_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` SHUOZI_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir SHUOZI_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="SHUOZI_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E SHUOZI_HOME like
        ...
        ``shuozi gateway restart`` with a /tmp SHUOZI_HOME exported; the
        ...
        must refuse ANY temp-dir SHUOZI_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "hermes-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="SHUOZI_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="SHUOZI_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="SHUOZI_HOME={tmp_path}/shuozi-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="SHUOZI_HOME=/home/alice/.hermes"\n'
    ...
    def test_accepts_unit_without_shuozi_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m shuozi_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="SHUOZI_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "shuozi gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_shuozi_home_for_target_user", lambda home: "/home/alice/.hermes")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("SHUOZI_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("SHUOZI_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("SHUOZI_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # SHUOZI_HOME resolves under the (pytest tmp) test SHUOZI_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test SHUOZI_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>SHUOZI_HOME</key>"
                "<string>/Users/alice/.hermes</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup shuozi gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: shuozi gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup shuozi gateway" in out
...
class TestSystemUnitHermesHome:
    """SHUOZI_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'SHUOZI_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: SHUOZI_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SHUOZI_HOME", "/root/.shuozi/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SHUOZI_HOME=/home/alice/.shuozi/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_shuozi_home(self, monkeypatch):
        # Custom SHUOZI_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SHUOZI_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'SHUOZI_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's SHUOZI_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        shuozi_home = str(gateway_cli.get_shuozi_home().resolve())
        assert f'SHUOZI_HOME={shuozi_home}' in unit


class TestHermesHomeForTargetUser:
    """Unit tests for _shuozi_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("SHUOZI_HOME", raising=False)

        result = gateway_cli._shuozi_home_for_target_user("/home/alice")
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._shuozi_home_for_target_user("/home/alice")
        assert result == "/home/alice/.shuozi/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("SHUOZI_HOME", "/opt/hermes")
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "shuozi gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_shuozi_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.shuozi should not produce a --profile flag."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        result = gateway_cli._profile_arg(str(shuozi_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.shuozi/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".hermes" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHUOZI_HOME", str(tmp_path / ".hermes"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile SHUOZI_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "shuozi"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.shuozi/profiles/mybot/subdir should NOT match — too deep."""
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_shuozi_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hermes" / "shuozi-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hermes" / "shuozi-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".hermes" / "shuozi-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".hermes" / "shuozi-agent"
        ...
        monkeypatch.setenv("SHUOZI_HOME", str(root_home / ".hermes"))
        monkeypatch.setattr(gateway_cli, "get_shuozi_home", lambda: root_home / ".hermes")
        ...
        # WorkingDirectory is anchored at the target user's SHUOZI_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.shuozi/shuozi-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'shuozi gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'shuozi gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'shuozi gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "shuozi gateway run" in out
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m shuozi_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_hermes_service_in_user_scope(self, tmp_path, monkeypatch):
        ...
        legacy = user_dir / "shuozi.service"
    ...
    def test_detects_legacy_hermes_service_in_system_scope(self, tmp_path, monkeypatch):
        ...
        legacy = system_dir / "shuozi.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_unrelated_hermes_service(self, tmp_path, monkeypatch):
        ...
        If a user has some other package named ``shuozi`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "shuozi.service").write_text(
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "shuozi.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "shuozi.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m shuozi_cli.main gateway run
          - python path/to/shuozi_cli/main.py gateway run
          - shuozi gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m shuozi_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/shuozi_cli/main.py gateway run",
        ...
        for i, execstart in enumerate(variants):
            name = f"shuozi.service" if i == 0 else f"shuozi.service"  # same name
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "shuozi.service" in out
        assert "shuozi gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "shuozi.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo shuozi gateway migrate-legacy" in out
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "shuozi.service"
        system_legacy = system_dir / "shuozi.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `shuozi gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import shuozi_cli.main as cli_main
        ...
        # public, just confirm that `shuozi gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import shuozi_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "shuozi_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shuozi_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``shuozi gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        ...
        assert "sudo shuozi gateway uninstall --system" in out
        assert "shuozi gateway install" in out
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``shuozi gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (SHUOZI_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_shuozi_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_shuozi_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # SHUOZI_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".hermes"
        monkeypatch.setattr(gateway_cli, "get_shuozi_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_shuozi_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_shuozi_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_shuozi_home(self, tmp_path, monkeypatch):
        import re
