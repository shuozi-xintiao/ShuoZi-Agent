"""Tests for shuozi backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.shuozi directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "shuozi_state.db").write_bytes(b"fake-state")
    ...
    # shuozi-agent repo (should be EXCLUDED)
    (root / "shuozi-agent").mkdir(exist_ok=True)
    (root / "shuozi-agent" / "run_agent.py").write_text("# big file\n")
    (root / "shuozi-agent" / ".git").mkdir()
    (root / "shuozi-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_shuozi_agent(self):
        from shuozi_cli.backup import _should_exclude
        assert _should_exclude(Path("shuozi-agent/run_agent.py"))
        assert _should_exclude(Path("shuozi-agent/.git/HEAD"))
    ...
    def test_includes_nested_shuozi_agent_in_skills(self):
        """skills/autonomous-ai-agents/shuozi-agent/ must NOT be excluded —
        only the root-level shuozi-agent/ repo is skipped."""
        from shuozi_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/shuozi-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/shuozi-agent/sub/item.txt"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import shuozi_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = shuozi_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, shuozi_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_shuozi_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include shuozi-agent/ directory."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        _make_hermes_tree(shuozi_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "shuozi-agent" in n]
            assert agent_files == [], f"shuozi-agent files leaked into backup: {agent_files}"

    def test_includes_nested_shuozi_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../shuozi-agent/ but NOT root shuozi-agent/."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        _make_hermes_tree(shuozi_home)

        # Add a nested shuozi-agent directory inside skills (like the real layout)
        nested = shuozi_home / "skills" / "autonomous-ai-agents" / "shuozi-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# ShuoZi Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root shuozi-agent must be excluded
            root_agent = [n for n in names if n.startswith("shuozi-agent/")]
            assert root_agent == [], f"root shuozi-agent leaked: {root_agent}"
            # Nested skill shuozi-agent must be included
            assert "skills/autonomous-ai-agents/shuozi-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/shuozi-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "config.yaml").write_text("model: test\n")
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside SHUOZI_HOME."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        _make_hermes_tree(shuozi_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(shuozi_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from shuozi_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only shuozi_state.db (old wrong name) is rejected."""
        from shuozi_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["shuozi_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into shuozi home."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (shuozi_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (shuozi_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (shuozi_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (shuozi_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .shuozi/ prefix if all entries share it."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".shuozi/config.yaml": "model: test\n",
            ".shuozi/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (shuozi_home / "config.yaml").read_text() == "model: test\n"
        assert (shuozi_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a shuozi backup."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (shuozi_home / "config.yaml").exists()
        # traversal file should NOT exist outside shuozi home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (shuozi_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (shuozi_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (shuozi_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (shuozi_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHUOZI_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from shuozi_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("SHUOZI_HOME", str(dst_home))
        ...
        # shuozi-agent should NOT be present
        assert not (dst_home / "shuozi-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from shuozi_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without shuozi markers fails validation."""
        import io
        from shuozi_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .shuozi/ prefix wrapping all entries."""
        import io
        from shuozi_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".shuozi/config.yaml", "test")
            zf.writestr(".shuozi/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".shuozi/"
...
class TestBackupEdgeCases:
    def test_nonexistent_shuozi_home(self, tmp_path, monkeypatch):
        """Backup exits when shuozi home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".hermes"
        monkeypatch.setenv("SHUOZI_HOME", str(fake_home))
    ...
    def test_empty_shuozi_home(self, tmp_path, monkeypatch):
        """Backup handles empty shuozi home (no files to back up)."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        # Only excluded dirs, no actual files
        (shuozi_home / "__pycache__").mkdir()
        (shuozi_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = shuozi_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = shuozi_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside shuozi root."""
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside shuozi home
        out_zip = shuozi_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        shuozi_home.mkdir()
        (shuozi_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (shuozi_home / "config.yaml").exists()
        assert (shuozi_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (shuozi_home / "profiles" / "coder" / "config.yaml").exists()
        assert (shuozi_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "shuozi -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from shuozi_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def shuozi_home(self, tmp_path):
        """Create a fake SHUOZI_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)
        assert snap_id is not None
        snap_dir = shuozi_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", shuozi_home=shuozi_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)
        db_copy = shuozi_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)
        assert (shuozi_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)
        with open(shuozi_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(shuozi_home=empty) is None

    def test_list_snapshots(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", shuozi_home=shuozi_home)
        id2 = create_quick_snapshot(label="second", shuozi_home=shuozi_home)

        snaps = list_quick_snapshots(shuozi_home=shuozi_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", shuozi_home=shuozi_home)
        snaps = list_quick_snapshots(limit=3, shuozi_home=shuozi_home)
        assert len(snaps) == 3

    def test_restore_config(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)

        (shuozi_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (shuozi_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, shuozi_home=shuozi_home)
        assert result is True
        assert "openrouter" in (shuozi_home / "config.yaml").read_text()

    def test_restore_state_db(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(shuozi_home=shuozi_home)

        conn = sqlite3.connect(str(shuozi_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, shuozi_home=shuozi_home)
    ...
    def test_restore_nonexistent(self, shuozi_home):
        from shuozi_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", shuozi_home=shuozi_home) is False

    def test_auto_prune(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", shuozi_home=shuozi_home)
        snaps = list_quick_snapshots(limit=100, shuozi_home=shuozi_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, shuozi_home):
        from shuozi_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", shuozi_home=shuozi_home)
        deleted = prune_quick_snapshots(keep=3, shuozi_home=shuozi_home)
        assert deleted == 7
        assert len(list_quick_snapshots(shuozi_home=shuozi_home)) == 3

    def test_snapshot_includes_pairing_directories(self, shuozi_home):
        ...
        (shuozi_home / "platforms" / "pairing").mkdir(parents=True)
        (shuozi_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (shuozi_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (shuozi_home / "pairing").mkdir()
        (shuozi_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (shuozi_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, shuozi_home):
        ...
        assert restore_quick_snapshot(snap_id, shuozi_home=shuozi_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, shuozi_home):
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``shuozi update``
    ...
    def test_creates_backup_under_backups_dir(self, shuozi_home):
        from shuozi_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(shuozi_home=shuozi_home)
        assert out is not None
        assert out.exists()
        assert out.parent == shuozi_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, shuozi_home):
        """Pre-update backup should include the same user data that
        ``shuozi backup`` would, and should exclude the same directories."""
        from shuozi_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(shuozi_home=shuozi_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # shuozi-agent repo excluded
        assert not any(n.startswith("shuozi-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, shuozi_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from shuozi_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(shuozi_home=shuozi_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(shuozi_home=shuozi_home)
    ...
    def test_rotation_keeps_only_n(self, shuozi_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(shuozi_home=shuozi_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (shuozi_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, shuozi_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from shuozi_cli.backup import create_pre_update_backup

        (shuozi_home / "backups").mkdir(exist_ok=True)
        manual = shuozi_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(shuozi_home=shuozi_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from shuozi_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(shuozi_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, shuozi_home):
        ...
        """
        from shuozi_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(shuozi_home=shuozi_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, shuozi_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, shuozi_home):
        ...
        first = create_pre_update_backup(shuozi_home=shuozi_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(shuozi_home=shuozi_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(shuozi_home=shuozi_home, keep=0)
    ...
    def test_skips_symlinked_files(self, shuozi_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside SHUOZI_HOME."""
        from shuozi_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def shuozi_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".hermes"
        root.mkdir()
        _make_hermes_tree(root)
        # Point SHUOZI_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SHUOZI_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for shuozi_cli.config + hermes_constants so they pick up SHUOZI_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("shuozi_cli.config") or mod == "shuozi_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, shuozi_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from shuozi_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "shuozi import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((shuozi_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, shuozi_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (shuozi_home / "backups").exists() or not list(
            (shuozi_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, shuozi_home, capsys):
        from shuozi_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, shuozi_home, capsys):
        ...
        import yaml
        (shuozi_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("shuozi_cli.config"):
    ...
    def test_config_disabled_is_silent(self, shuozi_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((shuozi_home / "backups").glob("pre-update-*.zip")) \
            if (shuozi_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, shuozi_home, capsys):
...
class TestPreMigrationBackup:
    ...
    ``shuozi claw migrate`` runs before mutating ~/.shuozi/."""
    ...
    def test_creates_backup_under_backups_dir(self, shuozi_home):
        from shuozi_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(shuozi_home=shuozi_home)
        assert out is not None
        assert out.exists()
    ...
    def test_backup_uses_shared_exclusion_rules(self, shuozi_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``shuozi backup`` / ``create_pre_update_backup`` — no drift."""
        from shuozi_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(shuozi_home=shuozi_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, shuozi_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hermes
        backup — `shuozi import` should accept it."""
        from shuozi_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(shuozi_home=shuozi_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, shuozi_home):
        from shuozi_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(shuozi_home=shuozi_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(shuozi_home=shuozi_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, shuozi_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(shuozi_home=shuozi_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((shuozi_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_shuozi_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.shuozi yet — nothing to back up."""
        from shuozi_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(shuozi_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, shuozi_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from shuozi_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(shuozi_home=shuozi_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(shuozi_home=shuozi_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`shuozi update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, shuozi_home: Path, label="pre-update"):
        from shuozi_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, shuozi_home=shuozi_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from shuozi_cli.backup import restore_cron_jobs_if_emptied
        shuozi_home = tmp_path / ".hermes"
        jobs_path = shuozi_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(shuozi_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, shuozi_home=shuozi_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = shuozi_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, shuozi_home=shuozi_home) is None
        assert restore_cron_jobs_if_emptied("", shuozi_home=shuozi_home) is None
