...
``"shuozi"`` anywhere (case-insensitive). That false-positived on unrelated
local Modelfiles such as ``hermes-brain:qwen3-14b-ctx16k`` — a tool-capable
Qwen3 wrapper that happens to live under the "shuozi" tag namespace.
...
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    ...
    assert _check_hermes_model_warning(model_name) == _SHUOZI_MODEL_WARNING
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Edge cases
        "",
        "shuozi",  # bare "shuozi" isn't the 3/4 family
        "hermes-brain",
