...
from shuozi_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from shuozi_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from shuozi_cli.codex_runtime_plugin_migration import _query_codex_plugins
...
class TestStripUnmanagedPluginTables:
    ...
    ``shuozi codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "shuozi_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: SHUOZI_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``SHUOZI_HOME``
    ...
    def test_tempdir_detector_accepts_real_shuozi_home(self):
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when SHUOZI_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "SHUOZI_HOME" not in env, (
            f"pytest-tempdir SHUOZI_HOME leaked into codex MCP entry: "
            f"{env.get('SHUOZI_HOME')!r}"
        )

    def test_real_shuozi_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate SHUOZI_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("SHUOZI_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("SHUOZI_HOME") == real_path

    def test_unset_shuozi_home_omits_env_key(self, monkeypatch):
        """When SHUOZI_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever SHUOZI_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "SHUOZI_HOME" not in env, (
            f"SHUOZI_HOME should not be set when env var is unset, got: "
            f"{env.get('SHUOZI_HOME')!r}"
        )
