"""Tests for shuozi_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SHUOZI_HOME/node and prepends ~/.local/bin to
...
def _make_hermes_node(shuozi_home: Path) -> Path:
    """Create a fake $SHUOZI_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = shuozi_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_hermes_node(fake_home):
    shuozi_home = fake_home / ".hermes"
    node_bin = _make_hermes_node(shuozi_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(shuozi_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(shuozi_home) == []
