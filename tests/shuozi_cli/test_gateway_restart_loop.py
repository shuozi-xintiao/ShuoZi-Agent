...
Covers:
- Defense 1: gateway stop/restart refuse when _SHUOZI_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "shuozi gateway restart",
        "shuozi gateway stop",
        "shuozi gateway start",
        "shuozi  gateway  restart",         # double spaces
    ...
    @pytest.mark.parametrize("text", [
        ...
        "shuozi cron list",
        "shuozi update",
        "shuozi config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hermes_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade shuozi then run shuozi gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify shuozi gateway stop/restart refuse when _SHUOZI_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SHUOZI_GATEWAY", raising=False)
