"""Tests for the `shuozi memory reset` CLI command.
...
- Profile-scoped reset (uses SHUOZI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SHUOZI_HOME with memory files."""
    shuozi_home = tmp_path / ".hermes"
    memories = shuozi_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.shuozi/shuozi-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `shuozi memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        shuozi_home = tmp_path / ".hermes"
        (shuozi_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SHUOZI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_shuozi_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
