"""Tests for shuozi_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_shuozi_home):
    """Drop the example-dashboard fixture into the per-test SHUOZI_HOME
    ...
    isolated ``SHUOZI_HOME``.

    The user-plugin source is preferred over a transient
    ``SHUOZI_BUNDLED_PLUGINS`` override because the bundled dir is
    ...
    from shuozi_cli import web_server
    ...
    #      and start serving requests against a torn-down SHUOZI_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints SHUOZI_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import shuozi_cli.web_server as ws

        monkeypatch.setenv("SHUOZI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("SHUOZI_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_shuozi_home):
        """Create a TestClient and isolate the state DB under the test SHUOZI_HOME."""
        try:
        ...
        import shuozi_state
        from shuozi_constants import get_shuozi_home
        from shuozi_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(shuozi_state, "DEFAULT_DB_PATH", get_shuozi_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("shuozi_state.SessionDB", _FakeDB)
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from shuozi_cli.web_server import app
    ...
    def test_elevenlabs_voices_unavailable_without_key(self, monkeypatch):
        import shuozi_cli.web_server as web_server
    ...
    def test_update_hermes_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn shuozi update")
        ...
        assert "docker pull nousresearch/shuozi-agent:latest" in data["message"]
        ...
        assert any("docker pull nousresearch/shuozi-agent:latest" in line for line in status_data["lines"])
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from shuozi_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from shuozi_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "shuozi_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a ShuoZi provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `shuozi import` doesn't auto-abort at the overwrite prompt."""
        import shuozi_cli.web_server as ws
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from shuozi_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from shuozi_cli.config import save_env_value
        from shuozi_cli.web_server import _SESSION_TOKEN
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from shuozi_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "ShuoZi Agent"}
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``shuozi gateway restart`` child (e.g. when a stale
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/shuozi_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import shuozi_cli.nous_subscription as ns
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from shuozi_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from shuozi_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``shuozi model`` custom flow. Regression for the desktop loop where a
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `shuozi model`), not the first curated paid entry."""
        import shuozi_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from shuozi_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from shuozi_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_shuozi_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from shuozi_constants import get_shuozi_home
        get_shuozi_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from shuozi_constants import get_shuozi_home
        import shuozi_cli.profiles as profiles_mod

        shuozi_home = get_shuozi_home()
        shuozi_home.mkdir(parents=True, exist_ok=True)
        (shuozi_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = shuozi_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_shuozi_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_hermes_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "shuozi setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        default_skill = get_shuozi_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = get_shuozi_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_shuozi_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_shuozi_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_shuozi_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_shuozi_home() / "profiles" / "builder"
        token = set_shuozi_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_shuozi_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import shuozi_cli.skills_config as skills_config
        import shuozi_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import shuozi_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from shuozi_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from shuozi_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from shuozi_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.shuozi/dashboard-themes/."""

    def test_returns_empty_when_dir_missing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SHUOZI_HOME", str(tmp_path))
        from shuozi_cli import web_server
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``shuozi_cli/web_server.py``).
    ...
    def _setup_test_client(self, monkeypatch, _isolate_shuozi_home):
        ...
        monkeypatch.setattr(
            shuozi_state, "DEFAULT_DB_PATH", get_shuozi_home() / "state.db"
        )
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in shuozi_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_shuozi_home):
        ...
        # Pin the SessionDB to the isolated SHUOZI_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_shuozi_home, _install_example_plugin):
        ...
        installs it into the per-test ``SHUOZI_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into SHUOZI_HOME by the class-level
...
# or ``sh -c 'printf …'``) instead of the real ``shuozi --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import shuozi_cli.main as main_mod
        ...
        assert env["SHUOZI_TUI_INLINE"] == "1"
        assert env["SHUOZI_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_shuozi_home
    ):
        import shuozi_cli.main as main_mod

        config_path = Path(os.environ["SHUOZI_HOME"]) / "config.yaml"
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from shuozi_cli.pty_bridge import PtyUnavailableError
        ...
        import shuozi_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        SHUOZI_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from shuozi_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("SHUOZI_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_shuozi_home, _install_example_plugin):
        ...
        the per-test ``SHUOZI_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_shuozi_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("SHUOZI_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under SHUOZI_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_shuozi_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("SHUOZI_DESKTOP", raising=False)
