...
``shuozi update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def test_recovery_runs_install_and_clears_marker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("shuozi_cli.managed_uv.ensure_uv", lambda: None)
    monkeypatch.setattr(
        m,
        "_install_python_dependencies_with_optional_fallback",
