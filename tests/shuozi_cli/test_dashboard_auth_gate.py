...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__SHUOZI_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("shuozi-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from shuozi_cli.web_server import should_require_auth
...
def test_start_server_public_without_insecure_records_auth_required(monkeypatch):
    ...
    """
    from shuozi_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set SHUOZI_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from shuozi_cli.dashboard_auth import clear_providers
    ...
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    monkeypatch.delenv("SHUOZI_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
    monkeypatch.delenv("SHUOZI_DASHBOARD_PORTAL_URL", raising=False)
    from unittest.mock import MagicMock
    nous_plugin.register(MagicMock())  # populates LAST_SKIP_REASON
    assert "SHUOZI_DASHBOARD_OAUTH_CLIENT_ID" in nous_plugin.LAST_SKIP_REASON
    ...
    assert "SHUOZI_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
