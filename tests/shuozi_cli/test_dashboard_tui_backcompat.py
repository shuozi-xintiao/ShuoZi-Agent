"""Regression test: `shuozi dashboard --tui` must not hard-crash.

Older Hermes desktop app shells (<= 0.15.x) spawn the backend as::

    shuozi dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
def _run_cli(args, timeout=60):
    """Invoke the real shuozi_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "shuozi_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
