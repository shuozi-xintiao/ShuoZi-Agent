...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `shuozi_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "shuozi"
    called = []

    fake_main_module = types.ModuleType("shuozi_cli.main")

    def fake_main():
        called.append("shuozi_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "shuozi_cli.main", fake_main_module)
