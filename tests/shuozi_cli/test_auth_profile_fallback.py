...
When ``SHUOZI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.shuozi/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hermes            (has its own auth.json fixture)
    * Profile     -> tmp_path/.shuozi/profiles/coder   (active, SHUOZI_HOME points here)
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from shuozi_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    shuozi_home = tmp_path / "classic"
    shuozi_home.mkdir()
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

    _write(shuozi_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SHUOZI_HOME == global root), no fallback path runs.
    ...
    # to the SHUOZI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    shuozi_home = tmp_path / "classic"
    ...
    _write(shuozi_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from shuozi_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SHUOZI_HOME is set to a custom path that is NOT under
    # ~/.shuozi/profiles/ — get_default_hermes_root() returns SHUOZI_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from shuozi_cli.auth import read_credential_pool, write_credential_pool
