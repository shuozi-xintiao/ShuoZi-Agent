...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SHUOZI_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SHUOZI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SHUOZI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from shuozi_cli.providers import SHUOZI_OVERLAYS
        assert "bedrock" in SHUOZI_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from shuozi_cli.providers import SHUOZI_OVERLAYS
        assert SHUOZI_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from shuozi_cli.providers import SHUOZI_OVERLAYS
        assert SHUOZI_OVERLAYS["bedrock"].auth_type == "aws_sdk"
