...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate SHUOZI_HOME for each test.

    The global hermetic fixture already redirects SHUOZI_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    SHUOZI_HOME here for clarity.
    """
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    monkeypatch.setenv("SHUOZI_HOME", str(hermes_home))
    yield hermes_home
...
class TestBundledDiscovery:
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from shuozi_cli import plugins as pmod
