...
- Path resolution (absolute, relative to SHUOZI_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SHUOZI_HOME."""
    shuozi_home = tmp_path / ".hermes"
    shuozi_home.mkdir()
    (shuozi_home / "cron").mkdir()
    (shuozi_home / "cron" / "output").mkdir()
    (shuozi_home / "scripts").mkdir()
    monkeypatch.setenv("SHUOZI_HOME", str(shuozi_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SHUOZI_DIR", shuozi_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", shuozi_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", shuozi_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", shuozi_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.shuozi/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SHUOZI_SESSION_PLATFORM") is None
        assert os.environ.get("SHUOZI_SESSION_CHAT_ID") is None
        assert os.environ.get("SHUOZI_SESSION_CHAT_NAME") is None
