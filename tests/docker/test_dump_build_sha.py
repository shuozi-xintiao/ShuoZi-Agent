"""Regression test: ``shuozi dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``shuozi dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$SHUOZI_GIT_SHA`` build-arg to ``/opt/hermes/.hermes_build_sha`` and
``shuozi_cli/build_info.py`` reads it as a fallback.
...
is absent and ``shuozi dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/hermes/.hermes_build_sha`` exists in the image, assert that
  ``shuozi dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's shuozi user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"shuozi dump exited {r.returncode}: "
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``SHUOZI_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``shuozi dump``
