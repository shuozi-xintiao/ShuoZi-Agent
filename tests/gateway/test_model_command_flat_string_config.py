...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "shuozi_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_shuozi_home() / config.yaml`` — point it here.
    monkeypatch.setattr("hermes_constants.get_shuozi_home", lambda: shuozi_home)
    monkeypatch.setattr("shuozi_cli.config.get_shuozi_home", lambda: shuozi_home)
    return cfg_path
