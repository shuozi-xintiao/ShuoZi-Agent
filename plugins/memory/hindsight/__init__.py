...
Or via $SHUOZI_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SHUOZI_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_shuozi_home() / "hindsight" / "config.json"
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $SHUOZI_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from shuozi_cli.memory_setup import _curses_select
