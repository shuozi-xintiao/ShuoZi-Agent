...
2. User-installed providers: ``$SHUOZI_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$SHUOZI_HOME/plugins/`` or None if unavailable."""
    try:
        from shuozi_constants import get_shuozi_home
        d = get_shuozi_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$SHUOZI_HOME/plugins/<name>/``) directories.  Bundled takes
