...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update ShuoZi Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `shuozi update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try SHUOZI_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
