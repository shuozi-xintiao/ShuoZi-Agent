...
// Terminal. Explicit SHUOZI_TUI_THEME / COLORFGBG signals above still win,
...
// `SHUOZI_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `SHUOZI_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `SHUOZI_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `SHUOZI_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
export function detectLightMode(
  ...
  const lightFlag = (env.SHUOZI_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.SHUOZI_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.SHUOZI_TUI_BACKGROUND ?? '')
