...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("SHUOZI_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from shuozi_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SHUOZI_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SHUOZI_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SHUOZI_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SHUOZI_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SHUOZI_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SHUOZI_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("SHUOZI_CRON_SESSION"):
        return False
    if env_var_enabled("SHUOZI_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $SHUOZI_HOME, or by the resolved absolute
# active profile home path such as /home/hermes/.shuozi/config.yaml. The
# resolved-absolute form is folded into the ~/.shuozi/ patterns at detection
...
# go stale when SHUOZI_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SHUOZI_ENV_PATH = (
    r'(?:~\/\.shuozi/|'
    r'(?:\$home|\$\{home\})/\.shuozi/|'
    r'(?:\$shuozi_home|\$\{shuozi_home\})/)'
    r'\.env\b'
)
# ~/.shuozi/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SHUOZI_ENV_PATH; matches the SHUOZI_HOME override form as
# well as ~/.shuozi/.
_SHUOZI_CONFIG_PATH = (
    r'(?:~\/\.shuozi/|'
    r'(?:\$home|\$\{home\})/\.shuozi/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart shuozi gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "shuozi update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `shuozi gateway restart`
    ...
    # In-place edit of a Hermes-managed security file (~/.shuozi/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SHUOZI_CONFIG_PATH}|{_SHUOZI_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SHUOZI_CONFIG_PATH}|{_SHUOZI_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SHUOZI_CONFIG_PATH}|{_SHUOZI_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.shuozi/ form so the Hermes config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/hermes/.shuozi/config.yaml`) rather than
    # ~, $HOME, or $SHUOZI_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live SHUOZI_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_shuozi_home(command)
    return command


def _rewrite_resolved_shuozi_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.shuozi/``.

    Resolves the active ``SHUOZI_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.shuozi/`` so the static ``_SHUOZI_CONFIG_PATH`` / ``_SHUOZI_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from shuozi_constants import get_shuozi_home
        home = get_shuozi_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate SHUOZI_HOME (e.g. "/" or "") rewriting
        ...
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.shuozi/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["SHUOZI_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SHUOZI_SPINNER_PAUSE" in os.environ:
            del os.environ["SHUOZI_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("SHUOZI_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set SHUOZI_INTERACTIVE or SHUOZI_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("SHUOZI_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("SHUOZI_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("SHUOZI_EXEC_ASK")
