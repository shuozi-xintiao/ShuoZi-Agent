...
Used by shuozi_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("shuozi", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.shuozi/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("shuozi", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
SHUOZI_INDEX_URL = "https://shuozi-agent.nousresearch.com/docs/api/skills-index.json"
SHUOZI_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "hermes-index.json"
SHUOZI_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for SHUOZI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if SHUOZI_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - SHUOZI_INDEX_CACHE_FILE.stat().st_mtime
            if age < SHUOZI_INDEX_TTL:
                return json.loads(SHUOZI_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(SHUOZI_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        SHUOZI_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        SHUOZI_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
