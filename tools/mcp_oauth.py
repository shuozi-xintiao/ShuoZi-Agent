    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "ShuoZi Agent"
"""
...
from shuozi_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses SHUOZI_HOME so each profile gets its own OAuth tokens.
    Layout: ``SHUOZI_HOME/mcp-tokens/``
    """
    try:
        from shuozi_constants import get_shuozi_home
        base = Path(get_shuozi_home())
    except ImportError:
        base = Path(os.environ.get("SHUOZI_HOME", str(Path.home() / ".hermes")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        SHUOZI_HOME/mcp-tokens/<server_name>.json         -- tokens
        SHUOZI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SHUOZI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://shuozi-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `shuozi mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "ShuoZi Agent")
    scope = cfg.get("scope")
