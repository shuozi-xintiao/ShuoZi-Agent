...
_SHUOZI_PROVIDER_ENV_FORCE_PREFIX = "_SHUOZI_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    blocked.update({
        ...
        "SHUOZI_DASHBOARD_SESSION_TOKEN",
...
_SHUOZI_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_shuozi_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from shuozi_constants import get_shuozi_home_override

        value = get_shuozi_home_override()
        if value:
            env["SHUOZI_HOME"] = value
    except Exception:
        pass
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_SHUOZI_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _SHUOZI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_SHUOZI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_SHUOZI_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _SHUOZI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_shuozi_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SHUOZI_GIT_BASH_PATH")
    ...
    _hermes_portable_git = os.path.join(_local_appdata, "shuozi", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. ShuoZi Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set SHUOZI_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SHUOZI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_SHUOZI_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _SHUOZI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        run_env[path_key] = _append_missing_sane_path_entries(run_env.get(path_key, ""))

    _inject_context_shuozi_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {SHUOZI_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SHUOZI_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SHUOZI_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_shuozi_home() / "cache" / "terminal"
            except Exception:
