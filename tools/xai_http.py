...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``shuozi tools`` repaint, tool-registration scans,
    ...
    2. ``~/.shuozi/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_shuozi_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.shuozi/.env`` first, then ``os.environ``.

    Wraps :func:`shuozi_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from shuozi_cli.config import get_env_value as _hermes_get_env_value
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``shuozi_cli.config.get_env_value`` so keys
    stored in ``~/.shuozi/.env`` (the standard Hermes location) are honored —
