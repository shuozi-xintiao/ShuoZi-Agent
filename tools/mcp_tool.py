...
transport, discovers their tools, and registers them into the shuozi-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.shuozi/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.shuozi/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            shuozi_home = os.path.expanduser(
                os.getenv(
                    "SHUOZI_HOME", os.path.join(os.path.expanduser("~"), ".hermes")
                )
            )
            candidates = [
                os.path.join(shuozi_home, "node", "bin", resolved_command),
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'shuozi-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'shuozi-agent[all]'"
            )
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `shuozi mcp login {server_name}` (or delete the tokens "
            f"file under ~/.shuozi/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local SHUOZI_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_shuozi_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_shuozi_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local SHUOZI_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_shuozi_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.shuozi/.env`` loaded at startup).
    """
    try:
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``shuozi tools`` interactive configuration — connects to each
