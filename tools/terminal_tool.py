...
# display_shuozi_home imported lazily at call site (stale-module safety during shuozi update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of shuozi directories
        total_bytes = 0
        import glob
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("SHUOZI_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("SHUOZI_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (SHUOZI_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (SHUOZI_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive ShuoZi password prompt or the sudo -S password-pipe path.
    ...
    should_prompt_for_sudo = (
        env_var_enabled("SHUOZI_INTERACTIVE") or has_sudo_prompt_callback
    )
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so ShuoZi can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``shuozi-agent=1`` for the current
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.shuozi/.env or environment variables."
        )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so ShuoZi can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # shuozi-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in shuozi-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("SHUOZI_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("SHUOZI_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("SHUOZI_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("SHUOZI_SESSION_USER_ID", "")
                        _gw_user_name = _gse("SHUOZI_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("SHUOZI_SESSION_MESSAGE_ID", "")
