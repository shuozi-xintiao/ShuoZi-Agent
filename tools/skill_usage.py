...
Tracks per-skill usage metadata in a sidecar JSON file (~/.shuozi/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.shuozi/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.shuozi/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.shuozi/skills/.curator_suppressed``. This is
    what makes pruning a built-in durable: without it, ``shuozi update`` would
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.shuozi/skills/.archive/``.

    Archive layout is flat (``.archive/<skill>/``) as set by ``archive_skill``,
    so the directory name is the skill name. Used by ``shuozi curator
    list-archived`` to help users pass a name to ``shuozi curator restore``.
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.shuozi/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.shuozi/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.shuozi/skills/<skill>/SKILL.md) and category-nested
    (~/.shuozi/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
