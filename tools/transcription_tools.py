...
LOCAL_STT_COMMAND_ENV = "SHUOZI_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "SHUOZI_LOCAL_STT_LANGUAGE"
...
#
# The single-env-var ``SHUOZI_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__SHUOZI_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set SHUOZI_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `shuozi model` or set XAI_API_KEY",
        }
    ...
    language = str(
        ...
        or os.getenv("SHUOZI_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
