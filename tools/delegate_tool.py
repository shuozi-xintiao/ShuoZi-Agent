...
# Must match shuozi_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.shuozi/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'shuozi auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (shuozi_cli/config.py load_config()) so that
