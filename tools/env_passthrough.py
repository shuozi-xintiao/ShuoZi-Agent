...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    token, or similar) per ``_SHUOZI_PROVIDER_ENV_BLOCKLIST``.
    ...
    try:
        ...
        return False
    return name in _SHUOZI_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_SHUOZI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register ShuoZi provider "
                "credential %r (blocked by _SHUOZI_PROVIDER_ENV_BLOCKLIST). "
            ...
            for item in passthrough:
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        ...
                        "by _SHUOZI_PROVIDER_ENV_BLOCKLIST). Operator "
