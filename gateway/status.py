...
The PID file lives at ``{SHUOZI_HOME}/gateway.pid``.  SHUOZI_HOME defaults to
``~/.shuozi`` but can be overridden via the environment variable.  This means
separate SHUOZI_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SHUOZI_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SHUOZI_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "shuozi" / _LOCKS_DIRNAME
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SHUOZI_HOME dirs).
    """
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``shuozi gateway stop`` on Windows would be misclassified as an
