...
from shuozi_constants import get_default_hermes_root, get_hermes_dir, get_shuozi_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.shuozi/.env manually."
)
...
# Default location: {SHUOZI_HOME}/cache/images/ (legacy: image_cache/)
...
_SHUOZI_ROOT = get_default_hermes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SHUOZI_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SHUOZI_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SHUOZI_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SHUOZI_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SHUOZI_HOME / "image_cache",
    _SHUOZI_HOME / "audio_cache",
    _SHUOZI_HOME / "video_cache",
    _SHUOZI_HOME / "document_cache",
    _SHUOZI_HOME / "browser_screenshots",
    ...
    _SHUOZI_HOME / "cache" / "images",
    _SHUOZI_HOME / "cache" / "audio",
    _SHUOZI_HOME / "cache" / "videos",
    _SHUOZI_HOME / "cache" / "documents",
    _SHUOZI_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active ShuoZi profile and shared Hermes root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for hermes_root in (_SHUOZI_HOME, _SHUOZI_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    secrets (``~/.shuozi/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``SHUOZI_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Hermes-managed cache, under an operator-allowlisted root
    (``SHUOZI_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.shuozi/.env,
    # ~/.shuozi/auth.json, etc. — so the obvious prompt-injection sites
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("SHUOZI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SHUOZI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SHUOZI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.shuozi/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from shuozi_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SHUOZI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SHUOZI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SHUOZI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SHUOZI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SHUOZI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SHUOZI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
