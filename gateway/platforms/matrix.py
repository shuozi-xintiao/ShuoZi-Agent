...
# Uses get_shuozi_home() so each profile gets its own Matrix store.
from shuozi_constants import get_hermes_dir as _get_hermes_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SHUOZI_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("SHUOZI_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "shuozi"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "ShuoZi Agent" become "Agent".
        """
        if not body:
            return ""
