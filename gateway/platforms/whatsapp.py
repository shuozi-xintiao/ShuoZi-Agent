...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    # Default bridge location relative to the shuozi-agent install
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `shuozi gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run shuozi whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `shuozi whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `shuozi whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `shuozi update` bumps the Baileys pin).  The stamp file records
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `shuozi update`, so without this check it
            ...
            # hardcodes ~/.shuozi/{image,audio,document}_cache, which diverges
            # under SHUOZI_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["SHUOZI_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["SHUOZI_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["SHUOZI_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: shuozi whatsapp")
